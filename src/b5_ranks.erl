...
delete_any(Key, #b5_ranks{root = Root, size = Size} = Tree) ->
    case b5_ranks_node:is_defined(Key, Root) of
        true ->
            Tree#b5_ranks{
                root = b5_ranks_node:delete(Key, Root),
                size = Size - 1
...
enter(Key, Value, #b5_ranks{size = Size, root = Root} = Tree) ->
    case b5_ranks_node:is_defined(Key, Root) of
        true ->
            UpdatedRoot = b5_ranks_node:update(Key, eager, Value, Root),
...
lookup(Key, #b5_ranks{root = Root}) ->
    b5_ranks_node:lookup(Key, Root).
...
take_any(Key, #b5_ranks{size = Size, root = Root} = Tree) ->
    case b5_ranks_node:is_defined(Key, Root) of
        true ->
            [TakenPair | UpdatedRoot] = b5_ranks_node:take(Key, Root),
            UpdatedTree = Tree#b5_ranks{size = Size - 1, root = UpdatedRoot},
...
update_with(Key, Fun, Init, #b5_ranks{root = Root} = Tree) ->
    case b5_ranks_node:lookup(Key, Root) of
        {value, PrevValue} ->
            UpdatedValue = Fun(PrevValue),
            UpdatedRoot = b5_ranks_node:update(Key, eager, UpdatedValue, Root),
            Tree#b5_ranks{root = UpdatedRoot};
        %
        none ->
            Tree#b5_ranks{
