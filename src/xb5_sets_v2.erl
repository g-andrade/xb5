...
See also:
- `m:xb5_sets_v2` for an element set supporting set operations (union, intersection, difference)
...
-opaque set(Element) :: #xb5_set{size :: non_neg_integer(), root :: xb5_sets_v2_node:t(Element)}.
-export_type([set/1]).
...
-opaque iter(Element) :: xb5_sets_v2_node:iter(Element).
-export_type([iter/1]).
...
-type unwrapped_set(Element) :: #{
    ...
    root := xb5_sets_v2_node:t(Element)
}.
...
```erlang
> S0 = xb5_sets_v2:new().
> xb5_sets_v2:to_list(xb5_sets_v2:add(1, S0)).
[1]
> S1 = xb5_sets_v2:from_list([1, 2, 3]).
> xb5_sets_v2:to_list(xb5_sets_v2:add(2, S1)).
[1, 2, 3]
> xb5_sets_v2:to_list(xb5_sets_v2:add(4, S1)).
[1, 2, 3, 4]
```
""".
...
add(Element, #xb5_set{size = Size, root = Root} = Set) ->
    case xb5_sets_v2_node:insert_att(Element, Root) of
        key_exists ->
            Set;
        %
        UpdatedRoot ->
...
```erlang
> S = xb5_sets_v2:from_list([3, 1, 2]).
> xb5_sets_v2:to_list(xb5_sets_v2:balance(S)).
[1, 2, 3]
```
""".
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> xb5_sets_v2:to_list(xb5_sets_v2:delete(2, S)).
[1, 3]
```
""".
...
delete(Element, #xb5_set{size = Size, root = Root} = Set) ->
    case xb5_sets_v2_node:delete_att(Element, Root) of
        badkey ->
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> xb5_sets_v2:to_list(xb5_sets_v2:delete_any(2, S)).
[1, 3]
> xb5_sets_v2:to_list(xb5_sets_v2:delete_any(42, S)).
[1, 2, 3]
```
""".
...
```erlang
> S1 = xb5_sets_v2:from_list([1, 2, 3, 4]).
> S2 = xb5_sets_v2:from_list([2, 4, 5]).
> xb5_sets_v2:to_list(xb5_sets_v2:difference(S1, S2)).
[1, 3]
```
""".
...
difference(#xb5_set{size = Size1, root = Root1}, #xb5_set{size = Size2, root = Root2}) ->
    [NewSize | NewRoot] = xb5_sets_v2_node:difference(Size1, Root1, Size2, Root2),
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3, 4, 5]).
> xb5_sets_v2:to_list(xb5_sets_v2:filter(fun(X) -> X > 3 end, S)).
[4, 5]
```
""".
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3, 4, 5]).
> xb5_sets_v2:to_list(xb5_sets_v2:filtermap(fun(X) when X rem 2 =:= 0 -> {true, X * 10}; (_) -> false end, S)).
[20, 40]
```
""".
...
filtermap(Fun, #xb5_set{root = Root}) ->
    FiltermappedList = xb5_sets_v2_node:filtermap_to_list(Fun, Root),
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> xb5_sets_v2:fold(fun(X, Acc) -> X + Acc end, 0, S).
6
```
""".
...
fold(Fun, Acc, #xb5_set{root = Root}) ->
    xb5_sets_v2_node:fold(Fun, Acc, Root).
...
```erlang
> xb5_sets_v2:to_list(xb5_sets_v2:from_list([3, 1, 2, 1])).
[1, 2, 3]
> xb5_sets_v2:to_list(xb5_sets_v2:from_list([])).
[]
```
""".
...
```erlang
> xb5_sets_v2:to_list(xb5_sets_v2:from_ordset([1, 2, 3])).
[1, 2, 3]
```
""".
...
from_ordset(Ordset) ->
    S = length(Ordset),
    Root = xb5_sets_v2_node:from_ordset(Ordset, S),
...
```erlang
> S0 = xb5_sets_v2:new().
> S1 = xb5_sets_v2:insert(1, S0).
> xb5_sets_v2:to_list(S1).
[1]
```
""".
...
```erlang
> S1 = xb5_sets_v2:from_list([1, 2, 3, 4]).
> S2 = xb5_sets_v2:from_list([2, 4, 5, 6]).
> xb5_sets_v2:to_list(xb5_sets_v2:intersection(S1, S2)).
[2, 4]
```
""".
...
intersection(#xb5_set{size = Size1, root = Root1}, #xb5_set{size = Size2, root = Root2}) ->
    [NewSize | NewRoot] = xb5_sets_v2_node:intersection(Size1, Root1, Size2, Root2),
    #xb5_set{size = NewSize, root = NewRoot}.
...
```erlang
> S1 = xb5_sets_v2:from_list([1, 2, 3]).
> S2 = xb5_sets_v2:from_list([2, 3, 4]).
> S3 = xb5_sets_v2:from_list([3, 4, 5]).
> xb5_sets_v2:to_list(xb5_sets_v2:intersection([S1, S2, S3])).
[3]
```
""".
...
```erlang
> S1 = xb5_sets_v2:from_list([1, 2, 3]).
> S2 = xb5_sets_v2:from_list([4, 5, 6]).
> xb5_sets_v2:is_disjoint(S1, S2).
true
> S3 = xb5_sets_v2:from_list([3, 4, 5]).
> xb5_sets_v2:is_disjoint(S1, S3).
false
```
""".
...
is_disjoint(#xb5_set{root = Root1, size = Size1}, #xb5_set{root = Root2, size = Size2}) ->
    xb5_sets_v2_node:is_disjoint(Root1, Size1, Root2, Size2).
...
```erlang
> xb5_sets_v2:is_empty(xb5_sets_v2:new()).
true
> xb5_sets_v2:is_empty(xb5_sets_v2:from_list([1])).
false
```
""".
...
```erlang
> S1 = xb5_sets_v2:from_list([1, 2, 3]).
> S2 = xb5_sets_v2:from_list([3, 1, 2]).
> xb5_sets_v2:is_equal(S1, S2).
true
> S3 = xb5_sets_v2:from_list([1, 2]).
> xb5_sets_v2:is_equal(S1, S3).
false
```
""".
...
is_equal(#xb5_set{size = Size1, root = Root1}, #xb5_set{size = Size2, root = Root2}) ->
    xb5_sets_v2_node:is_equal(Size1, Root1, Size2, Root2).
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> xb5_sets_v2:is_member(2, S).
true
> xb5_sets_v2:is_member(4, S).
false
```
""".
...
is_member(Element, #xb5_set{root = Root}) ->
    xb5_sets_v2_node:is_member(Element, Root).
...
```erlang
> xb5_sets_v2:is_set(xb5_sets_v2:new()).
true
> xb5_sets_v2:is_set(xb5_sets_v2:from_list([1, 2])).
true
> xb5_sets_v2:is_set(not_a_set).
false
```
""".
...
is_set(#xb5_set{size = Size, root = Root}) ->
    xb5_sets_v2_node:does_root_look_legit(Root, Size);
is_set(_) ->
    false.
...
```erlang
> S1 = xb5_sets_v2:from_list([1, 2]).
> S2 = xb5_sets_v2:from_list([1, 2, 3]).
> xb5_sets_v2:is_subset(S1, S2).
true
> xb5_sets_v2:is_subset(S2, S1).
false
```
""".
...
is_subset(#xb5_set{size = Size1, root = Root1}, #xb5_set{size = Size2, root = Root2}) ->
    xb5_sets_v2_node:is_subset(Size1, Root1, Size2, Root2).
...
```erlang
> S = xb5_sets_v2:from_list([3, 1, 2]).
> I = xb5_sets_v2:iterator(S).
> {1, I2} = xb5_sets_v2:next(I).
> {2, I3} = xb5_sets_v2:next(I2).
> {3, I4} = xb5_sets_v2:next(I3).
> xb5_sets_v2:next(I4).
none
```
""".
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> I = xb5_sets_v2:iterator(S, reversed).
> {3, I2} = xb5_sets_v2:next(I).
> {2, I3} = xb5_sets_v2:next(I2).
> {1, I4} = xb5_sets_v2:next(I3).
> xb5_sets_v2:next(I4).
none
```
""".
...
iterator(#xb5_set{root = Root}, Order) ->
    xb5_sets_v2_node:iterator(Root, Order).
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3, 4, 5]).
> I = xb5_sets_v2:iterator_from(3, S).
> {3, I2} = xb5_sets_v2:next(I).
> {4, I3} = xb5_sets_v2:next(I2).
> {5, I4} = xb5_sets_v2:next(I3).
> xb5_sets_v2:next(I4).
none
```
""".
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3, 4, 5]).
> I = xb5_sets_v2:iterator_from(3, S, reversed).
> {3, I2} = xb5_sets_v2:next(I).
> {2, I3} = xb5_sets_v2:next(I2).
> {1, I4} = xb5_sets_v2:next(I3).
> xb5_sets_v2:next(I4).
...
iterator_from(Element, #xb5_set{root = Root}, Order) ->
    xb5_sets_v2_node:iterator_from(Element, Root, Order).
...
```erlang
> S = xb5_sets_v2:from_list([1, 3, 5]).
> xb5_sets_v2:larger(2, S).
{found, 3}
> xb5_sets_v2:larger(5, S).
none
```
""".
...
larger(Element, #xb5_set{root = Root}) ->
    xb5_sets_v2_node:larger(Element, Root).
...
```erlang
> xb5_sets_v2:largest(xb5_sets_v2:from_list([1, 2, 3])).
3
```
""".
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> xb5_sets_v2:to_list(xb5_sets_v2:map(fun(X) -> X * 10 end, S)).
[10, 20, 30]
```
""".
...
map(Fun, #xb5_set{root = Root}) ->
    MappedList = xb5_sets_v2_node:map_to_list(Fun, Root),
...
> xb5_sets_v2:is_empty(xb5_sets_v2:new()).
true
> xb5_sets_v2:size(xb5_sets_v2:new()).
0
```
""".
-spec new() -> Set when
...
new() ->
    #xb5_set{size = 0, root = xb5_sets_v2_node:new()}.
...
```erlang
> xb5_sets_v2:to_list(xb5_sets_v2:singleton(42)).
[42]
> xb5_sets_v2:size(xb5_sets_v2:singleton(42)).
1
```
""".
...
singleton(Element) ->
    #xb5_set{size = 1, root = xb5_sets_v2_node:singleton(Element)}.
...
> xb5_sets_v2:size(xb5_sets_v2:new()).
0
> xb5_sets_v2:size(xb5_sets_v2:from_list([1, 2, 3])).
3
```
""".
...
```erlang
> S = xb5_sets_v2:from_list([1, 3, 5]).
> xb5_sets_v2:smaller(4, S).
{found, 3}
> xb5_sets_v2:smaller(1, S).
none
```
""".
...
smaller(Element, #xb5_set{root = Root}) ->
    xb5_sets_v2_node:smaller(Element, Root).
...
```erlang
> xb5_sets_v2:smallest(xb5_sets_v2:from_list([3, 1, 2])).
1
```
""".
...
smallest(#xb5_set{size = Size, root = Root}) when Size =/= 0 ->
    xb5_sets_v2_node:smallest(Root);
smallest(#xb5_set{}) ->
    error_empty_set().
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> Stats = xb5_sets_v2:structural_stats(S).
...
structural_stats(#xb5_set{root = Root}) ->
    xb5_sets_v2_node:structural_stats(Root).
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> {3, S2} = xb5_sets_v2:take_largest(S).
> xb5_sets_v2:to_list(S2).
[1, 2]
```
""".
...
take_largest(#xb5_set{size = Size, root = Root} = Set) when Size =/= 0 ->
    [Largest | UpdatedRoot] = xb5_sets_v2_node:take_largest(Root),
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> {1, S2} = xb5_sets_v2:take_smallest(S).
> xb5_sets_v2:to_list(S2).
[2, 3]
```
""".
...
take_smallest(#xb5_set{size = Size, root = Root} = Set) when Size =/= 0 ->
    [Smallest | UpdatedRoot] = xb5_sets_v2_node:take_smallest(Root),
...
```erlang
> xb5_sets_v2:to_list(xb5_sets_v2:from_list([3, 1, 2])).
[1, 2, 3]
> xb5_sets_v2:to_list(xb5_sets_v2:new()).
[]
```
""".
...
```erlang
> S1 = xb5_sets_v2:from_list([1, 2]).
> S2 = xb5_sets_v2:from_list([2, 3]).
> S3 = xb5_sets_v2:from_list([3, 4]).
> xb5_sets_v2:to_list(xb5_sets_v2:union([S1, S2, S3])).
[1, 2, 3, 4]
> xb5_sets_v2:to_list(xb5_sets_v2:union([])).
[]
```
""".
...
```erlang
> S1 = xb5_sets_v2:from_list([1, 2, 3]).
> S2 = xb5_sets_v2:from_list([3, 4, 5]).
> xb5_sets_v2:to_list(xb5_sets_v2:union(S1, S2)).
[1, 2, 3, 4, 5]
```
""".
...
union(#xb5_set{size = Size1, root = Root1}, #xb5_set{size = Size2, root = Root2}) ->
    [NewSize | NewRoot] = xb5_sets_v2_node:union(Size1, Root1, Size2, Root2),
    #xb5_set{size = NewSize, root = NewRoot}.
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> {ok, Unwrapped} = xb5_sets_v2:unwrap(S).
> maps:get(size, Unwrapped).
3
> {error, _} = xb5_sets_v2:unwrap(not_a_set).
```
""".
...
unwrap(#xb5_set{size = Size, root = Root} = Term) when is_integer(Size), Size >= 0 ->
    try xb5_sets_v2_node:structural_stats(Root) of
        Stats ->
...
```erlang
> S = xb5_sets_v2:from_list([1, 2, 3]).
> {ok, U} = xb5_sets_v2:unwrap(S).
> S2 = xb5_sets_v2:wrap(U).
> xb5_sets_v2:to_list(S2).
[1, 2, 3]
```
""".
