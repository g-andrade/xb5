...
filtermap(Fun, #xb5_set{root = Root}) ->
    [FilteredSize | FilteredRoot] = xb5_sets_v2_node:filtermap(Fun, Root),
...
is_equal(#xb5_set{root = Root1, size = Size1}, #xb5_set{root = Root2, size = Size2}) ->
    (Size1 =:= Size2) andalso xb5_sets_v2_node:is_equal(Root1, Root2).
...
map(Fun, #xb5_set{root = Root}) ->
    [NewSize | MappedRoot] = xb5_sets_v2_node:map(Fun, Root),
...
take_smallest(#xb5_set{size = Size, root = Root} = Set) when Size =/= 0 ->
    logger:notice("\n\n--------------\nTaking smallest at size ~p", [Size]),
