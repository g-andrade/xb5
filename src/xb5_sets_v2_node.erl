-module(xb5_sets_v2_node).

-moduledoc """
API for operating over `m:xb5_sets_v2` internal nodes directly.

> ℹ️
> You're likely looking for `m:xb5_sets_v2`.
...
-define(INTERNAL4_ARGS_IGN_C1, E1, E2, E3, E4, _, C2, C3, C4, C5).
-define(INTERNAL4_ARGS_IGN_C2, E1, E2, E3, E4, C1, _, C3, C4, C5).
-define(INTERNAL4_ARGS_IGN_C3, E1, E2, E3, E4, C1, C2, _, C4, C5).
-define(INTERNAL4_ARGS_IGN_C4, E1, E2, E3, E4, C1, C2, C3, _, C5).
-define(INTERNAL4_ARGS_IGN_C5, E1, E2, E3, E4, C1, C2, C3, C4, _).

-define(INTERNAL4_ARGS_EXCEPT_C1, E1, E2, E3, E4, C2, C3, C4, C5).
-define(INTERNAL4_ARGS_EXCEPT_C2, E1, E2, E3, E4, C1, C3, C4, C5).
-define(INTERNAL4_ARGS_EXCEPT_C3, E1, E2, E3, E4, C1, C2, C4, C5).
-define(INTERNAL4_ARGS_EXCEPT_C4, E1, E2, E3, E4, C1, C2, C3, C5).
-define(INTERNAL4_ARGS_EXCEPT_C5, E1, E2, E3, E4, C1, C2, C3, C4).
...
-define(INTERNAL3_ARGS_IGN_C1, E1, E2, E3, _, C2, C3, C4).
-define(INTERNAL3_ARGS_IGN_C2, E1, E2, E3, C1, _, C3, C4).
-define(INTERNAL3_ARGS_IGN_C3, E1, E2, E3, C1, C2, _, C4).
-define(INTERNAL3_ARGS_IGN_C4, E1, E2, E3, C1, C2, C3, _).

-define(INTERNAL3_ARGS_EXCEPT_C1, E1, E2, E3, C2, C3, C4).
-define(INTERNAL3_ARGS_EXCEPT_C2, E1, E2, E3, C1, C3, C4).
-define(INTERNAL3_ARGS_EXCEPT_C3, E1, E2, E3, C1, C2, C4).
-define(INTERNAL3_ARGS_EXCEPT_C4, E1, E2, E3, C1, C2, C3).
...
-define(INTERNAL2_ARGS_IGN_C1, E1, E2, _, C2, C3).
-define(INTERNAL2_ARGS_IGN_C2, E1, E2, C1, _, C3).
-define(INTERNAL2_ARGS_IGN_C3, E1, E2, C1, C2, _).

-define(INTERNAL2_ARGS_EXCEPT_C1, E1, E2, C2, C3).
-define(INTERNAL2_ARGS_EXCEPT_C2, E1, E2, C1, C3).
-define(INTERNAL2_ARGS_EXCEPT_C3, E1, E2, C1, C2).
...
-define(INTERNAL1_ARGS_IGN_C1, E1, _, C2).
-define(INTERNAL1_ARGS_IGN_C2, E1, C1, _).

-define(INTERNAL1_ARGS_EXCEPT_C1, E1, C2).
-define(INTERNAL1_ARGS_EXCEPT_C2, E1, C1).
...
% Cannot clash with any node layout
-define(REBALANCE(Type, ChildH, ChildS, ChildN, UpdatedChild), [
    Type, ChildH, ChildS, ChildN | UpdatedChild
]).

-define(REBALANCE_MATCH_ALL, [Type, ChildH, ChildS, ChildN | UpdatedChild]).

-define(REBALANCE_ARGS, Type, ChildH, ChildS, ChildN, UpdatedChild).
...
from_ordset([E1], 1) ->
    ?new_LEAF1(E1);
from_ordset(L, S) ->
    [BatchOffset, BatchSize | _Height] = from_ordset_initial_batch_params(S),
    AtRoot = true,
...
take_smallest(?INTERNAL1_MATCH_ALL) ->
    new_take_smallest_INTERNAL1(?INTERNAL1_ARGS);
take_smallest(?LEAF1_MATCH_ALL) ->
    ?TAKEN(E1, ?LEAF0);
take_smallest(Root) ->
    % take_smallest_recur(Root).
    new_take_smallest_recur(Root, _AtRoot = true).
...
from_ordset_initial_batch_params(S) when S >= 1 ->
    from_ordset_initial_batch_params(S, 1, 1, 1).

from_ordset_initial_batch_params(S, BatchOffset, BatchSize, Height) ->
    case BatchOffset + (4 * BatchSize) of
        NextOffset when NextOffset =< S ->
            from_ordset_initial_batch_params(S, NextOffset, BatchSize * 4, Height + 1);
        _ ->
            [BatchOffset, BatchSize | Height]
    end.
...
to_list_recur(Node, Acc) ->
    case Node of
        ...
        %
        ?INTERNAL2_MATCH_ALL ->
            to_list_INTERNAL2(?INTERNAL2_ARGS, Acc);
        %
        ?INTERNAL3_MATCH_ALL ->
            to_list_INTERNAL3(?INTERNAL3_ARGS, Acc);
        %
        ?INTERNAL4_MATCH_ALL ->
            to_list_INTERNAL4(?INTERNAL4_ARGS, Acc)
    end.

-compile({inline, to_list_INTERNAL2, ?INTERNAL2_ARITY_PLUS1}).
to_list_INTERNAL2(?INTERNAL2_ARGS, Acc) ->
...
-compile({inline, to_list_INTERNAL3, ?INTERNAL3_ARITY_PLUS1}).
to_list_INTERNAL3(?INTERNAL3_ARGS, Acc) ->
    % FIXME maybe not needed
...
-compile({inline, to_list_INTERNAL4, ?INTERNAL4_ARITY_PLUS1}).
to_list_INTERNAL4(?INTERNAL4_ARGS, Acc) ->
    % FIXME maybe not needed
...
%% ------------------------------------------------------------------
%% Internal Function Definitions: del_to_list/1
%% ------------------------------------------------------------------

del_to_list(Node) ->
    del_to_list_recur(Node, []).

del_to_list_recur(Node, Acc) ->
    case Node of
        ?LEAF2_MATCH_ALL ->
            [E1, E2 | Acc];
        %
        ...
        %
        ?INTERNAL2_MATCH_ALL ->
            del_to_list_INTERNAL2(?INTERNAL2_ARGS, Acc);
        %
        ?INTERNAL3_MATCH_ALL ->
            del_to_list_INTERNAL3(?INTERNAL3_ARGS, Acc);
        %
        ?INTERNAL4_MATCH_ALL ->
            del_to_list_INTERNAL4(?INTERNAL4_ARGS, Acc);
        %
        % FIXME
        %
        ?INTERNAL1_MATCH_ALL ->
            Acc3 = del_to_list_recur(C2, Acc),
            Acc4 = [E1 | Acc3],
            del_to_list_recur(C1, Acc4);
        %
        ?LEAF1_MATCH_ALL ->
            [E1 | Acc]
    end.

-compile({inline, del_to_list_INTERNAL2, ?INTERNAL2_ARITY_PLUS1}).
del_to_list_INTERNAL2(?INTERNAL2_ARGS, Acc) ->
    % FIXME maybe not needed
    Acc2 = del_to_list_recur(C3, Acc),
    Acc3 = del_to_list_recur(C2, [E2 | Acc2]),
    _Acc4 = del_to_list_recur(C1, [E1 | Acc3]).

-compile({inline, del_to_list_INTERNAL3, ?INTERNAL3_ARITY_PLUS1}).
del_to_list_INTERNAL3(?INTERNAL3_ARGS, Acc) ->
    % FIXME maybe not needed
    Acc2 = del_to_list_recur(C4, Acc),
    Acc3 = del_to_list_recur(C3, [E3 | Acc2]),
    Acc4 = del_to_list_recur(C2, [E2 | Acc3]),
    _Acc5 = del_to_list_recur(C1, [E1 | Acc4]).

-compile({inline, del_to_list_INTERNAL4, ?INTERNAL4_ARITY_PLUS1}).
del_to_list_INTERNAL4(?INTERNAL4_ARGS, Acc) ->
    % FIXME maybe not needed
    Acc2 = del_to_list_recur(C5, Acc),
    Acc3 = del_to_list_recur(C4, [E4 | Acc2]),
    Acc4 = del_to_list_recur(C3, [E3 | Acc3]),
    Acc5 = del_to_list_recur(C2, [E2 | Acc4]),
    _Acc6 = del_to_list_recur(C1, [E1 | Acc5]).
...
check_node_elements(LineNumber, Type, Node) ->
    % FIXME changed for the new deletion algorithm
    List = del_to_list(Node),
...
recur_node_type(Node) ->
    case Node of
        ...
        %
        % 
        % FIXME added these clauses for the new deletion algorithm
        ?LEAF1(_) ->
            'LEAF1';
        %
        ?INTERNAL1(_, _, _) ->
...
new_take_smallest_recur(Node, AtRoot) ->
    case Node of
        ?INTERNAL2_MATCH_ALL ->
            new_take_smallest_INTERNAL2(?INTERNAL2_ARGS, AtRoot);
        %
        ?INTERNAL3_MATCH_ALL ->
            new_take_smallest_INTERNAL3(?INTERNAL3_ARGS, AtRoot);
        %
        ?INTERNAL4_MATCH_ALL ->
            new_take_smallest_INTERNAL4(?INTERNAL4_ARGS, AtRoot);
        %
        ?LEAF2_MATCH_ALL ->
            ?TAKEN(E1, rebalance_if_not_at_root(AtRoot, required, 1, 1, 1, ?LEAF1(E2)));
        %
        ?LEAF3_MATCH_ALL ->
            ?TAKEN(E1, rebalance_if_not_at_root(AtRoot, preferred, 1, 2, 1, ?LEAF2(E2, E3)));
        %
        ?LEAF4_MATCH_ALL ->
...
new_take_smallest_INTERNAL4(?INTERNAL4_ARGS, AtRoot) ->
    ?TAKEN(Taken, Result) = new_take_smallest_recur(C1, false),

    case Result of
        ?REBALANCE_MATCH_ALL ->
            ?TAKEN(
                Taken,
                new_del_rebalance_INTERNAL4_C1(?INTERNAL4_ARGS_EXCEPT_C1, AtRoot, ?REBALANCE_ARGS)
            );
        %
        UpdatedC1 ->
            ?TAKEN(Taken, ?INTERNAL4_UPD_C1(UpdatedC1))
    end.
...
new_take_smallest_INTERNAL3(?INTERNAL3_ARGS, AtRoot) ->
    ...
    case Result of
        ?REBALANCE_MATCH_ALL ->
            ?TAKEN(
                Taken,
                new_del_rebalance_INTERNAL3_C1(?INTERNAL3_ARGS_EXCEPT_C1, AtRoot, ?REBALANCE_ARGS)
            );
        %
        UpdatedC1 ->
            ?TAKEN(Taken, ?INTERNAL3_UPD_C1(UpdatedC1))
    end.
...
new_take_smallest_INTERNAL2(?INTERNAL2_ARGS, AtRoot) ->
    ...
    case Result of
        ?REBALANCE_MATCH_ALL ->
            ?TAKEN(
                Taken,
                new_del_rebalance_INTERNAL2_C1(?INTERNAL2_ARGS_EXCEPT_C1, AtRoot, ?REBALANCE_ARGS)
            );
        %
        UpdatedC1 ->
            ?TAKEN(Taken, ?INTERNAL2_UPD_C1(UpdatedC1))
    end.
...
new_take_smallest_INTERNAL1(?INTERNAL1_ARGS) ->
    ...
    case Result of
        ?REBALANCE_MATCH_ALL ->
            ?TAKEN(
                Taken,
                new_del_rebalance_INTERNAL1_C1(?INTERNAL1_ARGS_EXCEPT_C1, ?REBALANCE_ARGS)
            );
        %
        UpdatedC1 ->
            ?TAKEN(Taken, ?INTERNAL1_UPD_C1(UpdatedC1))
    end.

%%%%%%%%%%%%

rebalance_if_not_at_root(false, Type, ChildH, ChildS, ChildN, UpdatedChild) ->
    ?REBALANCE(Type, ChildH, ChildS, ChildN, UpdatedChild);
rebalance_if_not_at_root(true, _, _, _, _, UpdatedChild) ->
    UpdatedChild.
...
new_del_rebalance_INTERNAL4_C1(
  ?INTERNAL4_ARGS_EXCEPT_C1, AtRoot, Type, ChildH, S1, N1, UpdatedC1
) ->
    H = ChildH + 1,

    if
        H =:= 2 andalso Type =:= required ->
            case rebalance_leaf_from_right_sibling(UpdatedC1, E1, C2) of
                ?ROTATED(UpElem, RebalancedC1, UpdatedC2) ->
                    UpdatedNode = ?new_INTERNAL4(
                        UpElem,
                        E2,
                        E3,
                    ...
                    [S | N] = remaining_counts_INTERNAL4(S1, N1, UpdatedC2, C3, C4, C5),
                    new_del_rebalance(UpdatedNode, AtRoot, preferred, H, S, N);
                %
                ?MERGED(MergedC1C2) ->
                    UpdatedNode = ?new_INTERNAL3(E2, E3, E4, MergedC1C2, C3, C4, C5),
                    [S | N] = remaining_counts_INTERNAL3(4, 1, C3, C4, C5),
                    new_del_rebalance(UpdatedNode, AtRoot, preferred, H, S, N)
            end;
        %
        true ->
            UpdatedNode = ?INTERNAL4_UPD_C1(UpdatedC1),
            [S | N] = remaining_counts_INTERNAL4(S1, N1, C2, C3, C4, C5),
            new_del_rebalance(UpdatedNode, AtRoot, Type, H, S, N)
    end.

%% C1

new_del_rebalance_INTERNAL3_C1(
  ?INTERNAL3_ARGS_EXCEPT_C1, AtRoot, Type, ChildH, S1, N1, UpdatedC1
) ->
    H = ChildH + 1,
    ...
    if
        H =:= 2 andalso Type =:= required ->
            case rebalance_leaf_from_right_sibling(UpdatedC1, E1, C2) of
                ?ROTATED(UpElem, RebalancedC1, UpdatedC2) ->
                    UpdatedNode = ?new_INTERNAL3(UpElem, E2, E3, RebalancedC1, UpdatedC2, C3, C4),
                    [S | N] = remaining_counts_INTERNAL3(S1, N1, UpdatedC2, C3, C4),
                    new_del_rebalance(UpdatedNode, AtRoot, preferred, H, S, N);
                %
                ?MERGED(MergedC1C2) ->
                    UpdatedNode = ?new_INTERNAL2(E2, E3, MergedC1C2, C3, C4),
                    [S | N] = remaining_counts_INTERNAL2(4, 1, C3, C4),
                    new_del_rebalance(UpdatedNode, AtRoot, preferred, H, S, N)
            end;
        %
        true ->
            UpdatedNode = ?INTERNAL3_UPD_C1(UpdatedC1),
            [S | N] = remaining_counts_INTERNAL3(S1, N1, C2, C3, C4),
            new_del_rebalance(UpdatedNode, AtRoot, Type, H, S, N)
    end.
...
new_del_rebalance_INTERNAL2_C1(
  ?INTERNAL2_ARGS_EXCEPT_C1, AtRoot, Type, ChildH, S1, N1, UpdatedC1
) ->
    H = ChildH + 1,
    ...
    if
        H =:= 2 andalso Type =:= required ->
            case rebalance_leaf_from_right_sibling(UpdatedC1, E1, C2) of
                ?ROTATED(UpElem, RebalancedC1, UpdatedC2) ->
                    UpdatedNode = ?new_INTERNAL2(UpElem, E2, RebalancedC1, UpdatedC2, C3),
                    [S | N] = remaining_counts_INTERNAL2(S1, N1, UpdatedC2, C3),
                    new_del_rebalance(UpdatedNode, AtRoot, preferred, H, S, N);
                %
                ?MERGED(MergedC1C2) ->
                    UpdatedNode = ?new_INTERNAL1(E2, MergedC1C2, C3),
                    [S | N] = remaining_counts_INTERNAL1(4, 1, C3),
                    new_del_rebalance(UpdatedNode, AtRoot, required, H, S, N)
            end;
        %
        true ->
            UpdatedNode = ?INTERNAL2_UPD_C1(UpdatedC1),
            [S | N] = remaining_counts_INTERNAL2(S1, N1, C2, C3),
            new_del_rebalance(UpdatedNode, AtRoot, Type, H, S, N)
    end.

%%% C2
%
%new_del_rebalance_INTERNAL2_C2(?INTERNAL2_ARGS_EXCEPT_C2, AtRoot, Type, ChildH, S2, N2, UpdatedC2) ->
%    H = ChildH + 1,
%
%    if
%        H =:= 2 andalso Type =:= required ->
%            case rebalance_leaf_from_left_sibling(UpdatedC2, E1, C1) of
%                ?ROTATED(UpElem, UpdatedC1, RebalancedC2) ->
%                    UpdatedNode = ?new_INTERNAL2(UpElem, E2, UpdatedC1, RebalancedC2, C3),
%                    [S | N] = remaining_counts_INTERNAL2(S2, N2, UpdatedC1, C3),
%                    new_del_rebalance(UpdatedNode, AtRoot, preferred, H, S, N);
%                %
%                ?MERGED(MergedC1C2) ->
%                    UpdatedNode = ?new_INTERNAL1(E2, MergedC1C2, C3),
...
%        %
%        true ->
%            UpdatedNode = ?INTERNAL2_UPD_C2(UpdatedC2),
%            [S | N] = remaining_counts_INTERNAL2(S2, N2, C1, C3),
%            new_del_rebalance(UpdatedNode, AtRoot, Type, H, S, N)
%    end.
%
%%% C3
%
%new_del_rebalance_INTERNAL2_C3(?INTERNAL2_ARGS_EXCEPT_C3, AtRoot, Type, ChildH, S3, N3, UpdatedC3) ->
%    H = ChildH + 1,
%
%    if
%        H =:= 2 andalso Type =:= required ->
%            case rebalance_leaf_from_left_sibling(UpdatedC3, E2, C2) of
%                ?ROTATED(UpElem, UpdatedC2, RebalancedC3) ->
%                    UpdatedNode = ?new_INTERNAL2(E1, UpElem, C1, UpdatedC2, RebalancedC3),
%                    [S | N] = remaining_counts_INTERNAL2(S3, N3, C1, UpdatedC2),
%                    new_del_rebalance(UpdatedNode, AtRoot, preferred, H, S, N);
%                %
%                ?MERGED(MergedC2C3) ->
%                    UpdatedNode = ?new_INTERNAL1(E1, C1, MergedC2C3),
%                    [S | N] = remaining_counts_INTERNAL1(4, 1, C1),
%                    new_del_rebalance(UpdatedNode, AtRoot, required, H, S, N)
%            end;
%        %
%        true ->
%            UpdatedNode = ?INTERNAL2_UPD_C3(UpdatedC3),
%            [S | N] = remaining_counts_INTERNAL2(S3, N3, C1, C2),
%            new_del_rebalance(UpdatedNode, AtRoot, Type, H, S, N)
%    end.
...
new_del_rebalance_INTERNAL1_C1(?INTERNAL1_ARGS_EXCEPT_C1, Type, ChildH, S1, N1, UpdatedC1) ->
    H = ChildH + 1,
    AtRoot = true,
    ...
    if
        H =:= 2 andalso Type =:= required ->
            case rebalance_leaf_from_right_sibling(UpdatedC1, E1, C2) of
                ?ROTATED(UpElem, RebalancedC1, UpdatedC2) ->
                    ?new_INTERNAL1(UpElem, RebalancedC1, UpdatedC2);
                %
                ?MERGED(MergedC1C2) ->
                    % Height reduction
                    MergedC1C2
        ...
        %
        true ->
            UpdatedNode = ?INTERNAL1_UPD_C1(UpdatedC1),
            [S | N] = remaining_counts_INTERNAL1(S1, N1, C2),
            new_del_rebalance(UpdatedNode, AtRoot, Type, H, S, N)
    end.

%%%%%%%%%

remaining_counts_INTERNAL1(UpdatedChildS, UpdatedChildN, Sibling1) ->
    [SSibling1 | NSibling1] = count_recur(Sibling1),
    S = 1 + UpdatedChildS + SSibling1,
    N = 1 + UpdatedChildN + NSibling1,
    [S | N].

remaining_counts_INTERNAL2(UpdatedChildS, UpdatedChildN, Sibling1, Sibling2) ->
    [SSibling1 | NSibling1] = count_recur(Sibling1),
    [SSibling2 | NSibling2] = count_recur(Sibling2),
    S = 2 + UpdatedChildS + SSibling1 + SSibling2,
    N = 1 + UpdatedChildN + NSibling1 + NSibling2,
    [S | N].

remaining_counts_INTERNAL3(UpdatedChildS, UpdatedChildN, Sibling1, Sibling2, Sibling3) ->
    [SSibling1 | NSibling1] = count_recur(Sibling1),
    [SSibling2 | NSibling2] = count_recur(Sibling2),
    [SSibling3 | NSibling3] = count_recur(Sibling3),
    S = 3 + UpdatedChildS + SSibling1 + SSibling2 + SSibling3,
    N = 1 + UpdatedChildN + NSibling1 + NSibling2 + NSibling3,
    [S | N].

remaining_counts_INTERNAL4(UpdatedChildS, UpdatedChildN, Sibling1, Sibling2, Sibling3, Sibling4) ->
    [SSibling1 | NSibling1] = count_recur(Sibling1),
    [SSibling2 | NSibling2] = count_recur(Sibling2),
    [SSibling3 | NSibling3] = count_recur(Sibling3),
    [SSibling4 | NSibling4] = count_recur(Sibling4),
    S = 4 + UpdatedChildS + SSibling1 + SSibling2 + SSibling3 + SSibling4,
    N = 1 + UpdatedChildN + NSibling1 + NSibling2 + NSibling3 + NSibling4,
    [S | N].

%%%%%%%%%

new_del_rebalance(Node, AtRoot, Type, H, S, N) when
    Type =:= preferred, (S < 15 orelse S / N >= 2.642857142857143)
->
    % Key density is high enough, don't propagate wish to rebalance upwards
    logger:notice(
      "[AtRoot ~p, H=~p] Skipping rebuild at size ~p, enough key density or node too small", 
      [AtRoot, H, S]
    ),
    Node;
new_del_rebalance(Node, AtRoot, Type, H, S, N) ->
    [BatchOffset, BatchSize | IdealH] = from_ordset_initial_batch_params(S),

    case can_rebalance(AtRoot, Type, H, S, BatchOffset, BatchSize, IdealH) of
        true ->
            L = del_to_list(Node),
            logger:notice("[AtRoot ~p, H=~p, IdealH=~p] Rebuilding, ~p for size ~p", [AtRoot, H, IdealH, Type, S]),
            [RebalancedNode | []] = from_ordset_recur(L, S, BatchOffset, BatchSize, AtRoot),
            RebalancedNode;
        %
        false when AtRoot ->
            logger:notice("[AtRoot ~p, H=~p, IdealH=~p] Cannot rebuild ~p for size ~p", [AtRoot, H, IdealH, Type, S]),
            Node;
        %
        false ->
            logger:notice("[AtRoot ~p, H=~p, IdealH=~p] Cannot rebuild ~p for size ~p", [AtRoot, H, IdealH, Type, S]),
            ?REBALANCE(Type, H, S, N, Node)
    end.

-compile({inline, can_rebalance/7}).
can_rebalance(AtRoot, Type, CurrentH, S, BatchOffset, BatchSize, IdealH) ->
    Splits = (S - BatchOffset) div BatchSize,

    case Type of
        required ->
            AtRoot orelse (IdealH =:= CurrentH andalso Splits =/= 0);
        %
        preferred when AtRoot ->
            IdealH =< CurrentH;
        %
        preferred ->
            IdealH =:= CurrentH andalso Splits =/= 0
    end.
...
count_recur(Node) ->
    case Node of
        ?INTERNAL2_MATCH(_, _, C1, C2, C3) ->
            count_INTERNAL2(C1, C2, C3);
        %
        ?INTERNAL3_MATCH(_, _, _, C1, C2, C3, C4) ->
            count_INTERNAL3(C1, C2, C3, C4);
        %
        ?INTERNAL4_MATCH(_, _, _, _, C1, C2, C3, C4, C5) ->
            count_INTERNAL4(C1, C2, C3, C4, C5);
        %
        ?LEAF2(_, _) ->
            [2 | 1];
        %
        ...
        %
        ?INTERNAL1_MATCH(_, C1, C2) ->
            count_INTERNAL1(C1, C2);
        %
        ?LEAF1(_) ->
            [1 | 1]
    end.

-compile({inline, count_INTERNAL1/2}).
count_INTERNAL1(C1, C2) ->
    [S1 | N1] = count_recur(C1),
    [S2 | N2] = count_recur(C2),
    S = 1 + S1 + S2,
    N = 1 + N1 + N2,
    [S | N].

-compile({inline, count_INTERNAL2/3}).
count_INTERNAL2(C1, C2, C3) ->
    [S1 | N1] = count_recur(C1),
    [S2 | N2] = count_recur(C2),
    [S3 | N3] = count_recur(C3),
    S = 2 + S1 + S2 + S3,
    N = 1 + N1 + N2 + N3,
    [S | N].

-compile({inline, count_INTERNAL3/4}).
count_INTERNAL3(C1, C2, C3, C4) ->
    [S1 | N1] = count_recur(C1),
    [S2 | N2] = count_recur(C2),
    [S3 | N3] = count_recur(C3),
    [S4 | N4] = count_recur(C4),
    S = 3 + S1 + S2 + S3 + S4,
    N = 1 + N1 + N2 + N3 + N4,
    [S | N].

-compile({inline, count_INTERNAL4/5}).
count_INTERNAL4(C1, C2, C3, C4, C5) ->
    [S1 | N1] = count_recur(C1),
    [S2 | N2] = count_recur(C2),
    [S3 | N3] = count_recur(C3),
    [S4 | N4] = count_recur(C4),
    [S5 | N5] = count_recur(C5),
    S = 4 + S1 + S2 + S3 + S4 + S5,
    N = 1 + N1 + N2 + N3 + N4 + N5,
    [S | N].
...
rebalance_leaf_from_right_sibling(?LEAF1(CElem), RParentE, Right) ->
    case Right of
        ?LEAF2_MATCH_ALL ->
            MergedNode = ?new_LEAF4(
                CElem,
...
rebalance_leaf_from_left_sibling(?LEAF1(CElem), LParentE, Left) ->
    case Left of
        ?LEAF2_MATCH_ALL ->
            MergedNode = ?new_LEAF4(
                E1,
