...
-define(INTERNAL4_C1_REBALANCE(UpdatedC1, AtRoot),
    del_rebalance_INTERNAL4_C1(E1, E2, E3, E4, UpdatedC1, C2, C3, C4, C5, AtRoot)
).
...
delete_att(Elem, Root) ->
    delete_att_recur(Elem, Root, true).
...
take_smallest(?INTERNAL1_MATCH_ALL) ->
    ?TAKEN(Taken, UpdatedC1) = take_smallest_recur(C1, false),
    ?TAKEN(Taken, ?INTERNAL1_C1_REBALANCE(UpdatedC1));
take_smallest(?LEAF1_MATCH_ALL) ->
    ?TAKEN(E1, ?LEAF0);
take_smallest(Root) ->
    take_smallest_recur(Root, true).
...
delete_att_recur(Elem, Node, AtRoot) ->
    case Node of
        ?INTERNAL2_MATCH_ALL ->
        ...
        %
        ?INTERNAL4_MATCH_ALL ->
            delete_att_INTERNAL4(Elem, ?INTERNAL4_ARGS, AtRoot);
        %
        ?LEAF2_MATCH_ALL ->
...
delete_att_INTERNAL4(Elem, ?INTERNAL4_ARGS, AtRoot) ->
    ?GAP_SEARCH4(
        ...
        %
        delete_att_INTERNAL4_C1(Elem, ?INTERNAL4_ARGS, AtRoot),
...
delete_att_INTERNAL4_E1(?INTERNAL4_ARGS_IGN_E1) ->
    ?TAKEN(ReplacementE, UpdatedC2) = take_smallest_recur(C2, false),
...
delete_att_INTERNAL4_E3(?INTERNAL4_ARGS_IGN_E3) ->
    ?TAKEN(ReplacementE, UpdatedC4) = take_smallest_recur(C4, false),
...
delete_att_INTERNAL3_E2(?INTERNAL3_ARGS_IGN_E2) ->
    ?TAKEN(ReplacementE, UpdatedC3) = take_smallest_recur(C3, false),
...
take_smallest_recur(Node, AtRoot) ->
    case Node of
        ?INTERNAL2_MATCH_ALL ->
            ?TAKEN(Taken, UpdatedC1) = take_smallest_recur(C1, false),
        ...
        ?INTERNAL4_MATCH_ALL ->
            ?TAKEN(Taken, UpdatedC1) = take_smallest_recur(C1, false),
            ?TAKEN(Taken, ?INTERNAL4_C1_REBALANCE(UpdatedC1, AtRoot));
        %
        ?LEAF2_MATCH_ALL ->
...
del_rebalance_INTERNAL4_C1(?INTERNAL4_ARGS, AtRoot) ->
    case should_rebalance(C1) of
        badkey ->
            badkey;
        %
        true ->
            rebalance(?INTERNAL4_UPD_C1(C1), 4 + count(C1) + count(C2) + count(C3) + count(C4) + count(C5), AtRoot);
        %
        _ ->
            ?INTERNAL4_UPD_C1(C1)
    end.
...
should_rebalance(UpdatedChild) ->
    case UpdatedChild of
        badkey ->
            badkey;
        %
        ?INTERNAL1(_, _, _) ->
...
count(?INTERNAL4_MATCH(_, _, _, _, C1, C2, C3, C4, C5)) ->
    4 + count(C1) + count(C2) + count(C3) + count(C4) + count(C5);
count(?INTERNAL3_MATCH(_, _, _, C1, C2, C3, C4)) ->
    3 + count(C1) + count(C2) + count(C3) + count(C4);
count(?INTERNAL2_MATCH(_, _, C1, C2, C3)) ->
    2 + count(C1) + count(C2) + count(C3);
count(?INTERNAL1_MATCH(_, C1, C2)) ->
    1 + count(C1) + count(C2);
count(?LEAF4_MATCH(_, _, _, _)) ->
    4;
count(?LEAF3_MATCH(_, _, _)) ->
    3;
count(?LEAF2_MATCH(_, _)) ->
    2;
count(?LEAF1_MATCH(_)) ->
    1.
...
rebalance(Node, S, AtRoot) ->
    [BatchOffset | BatchSize] = xb5_utils:bulk_construction_params(S),

    L = to_list_recur(Node, []),

    [UpdatedNode | []] = from_ordset_recur(L, S, BatchOffset, BatchSize, AtRoot),
    UpdatedNode.
