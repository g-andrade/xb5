...
-record(b5_ranks_v1, {size, root}).

-opaque tree(Key, Value) :: #b5_ranks_v1{
    size :: non_neg_integer(),
    root :: b5_ranks_v1_node:t(Key, Value)
}.
-export_type([tree/2]).

%%%%%%%

-type iter(Key, Value) :: b5_ranks_v1_node:iter(Key, Value).
-export_type([iter/2]).
...
delete(Key, #b5_ranks_v1{root = Root, size = Size} = Tree) ->
    Tree#b5_ranks_v1{
        root = b5_ranks_v1_node:delete(Key, Root),
        size = Size - 1
    }.
...
enter(Key, Value, #b5_ranks_v1{size = Size, root = Root} = Tree) ->
    try b5_ranks_v1_node:update(Key, eager, Value, Root) of
        UpdatedRoot ->
            Tree#b5_ranks_v1{root = UpdatedRoot}
    catch
        error:{badkey, K} when K =:= Key ->
            Tree#b5_ranks_v1{
                root = b5_ranks_v1_node:insert(Key, eager, Value, Root),
                size = Size + 1
            }
    end.
...
foldl(Fun, Acc0, #b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:foldl(Fun, Acc0, Root).
...
-spec foldr(fun((Key, Value, Acc1) -> Acc2), Acc0, tree(Key, Value)) -> AccN when
    AccN :: Acc2, Acc2 :: Acc1, Acc1 :: Acc0.
foldr(Fun, Acc0, #b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:foldr(Fun, Acc0, Root).
...
get(Key, #b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:get(Key, Root).
...
insert(Key, Value, #b5_ranks_v1{size = Size, root = Root} = Tree) ->
    Tree#b5_ranks_v1{
        size = Size + 1,
        root = b5_ranks_v1_node:insert(Key, eager, Value, Root)
    }.
...
insert_with(Key, Fun, #b5_ranks_v1{size = Size, root = Root} = Tree) ->
    Tree#b5_ranks_v1{
        size = Size + 1,
        root = b5_ranks_v1_node:insert(Key, lazy, Fun, Root)
    }.
...
is_empty(#b5_ranks_v1{size = Size}) ->
    Size =:= 0.
...
-spec iterator(Tree) -> Iter when
    Tree :: tree(Key, Value),
    Iter :: b5_ranks_v1_node:iter(Key, Value).
...
iterator(#b5_ranks_v1{root = Root}, Order) ->
    b5_ranks_v1_node:iterator(Root, Order).
...
iterator_from(Key, #b5_ranks_v1{root = Root}, Order) ->
    b5_ranks_v1_node:iterator_from(Key, Root, Order).

-if(?OTP_RELEASE >= 27).
-doc "Returns the keys in `Tree` as an ordered list.".
-endif.
-spec keys(Tree) -> [Key] when Tree :: tree(Key, _).
keys(#b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:keys(Root).
...
larger(Key, #b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:larger(Key, Root).
...
largest(#b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:largest(Root).
...
map(Fun, #b5_ranks_v1{root = Root} = Tree) ->
    Tree#b5_ranks_v1{root = b5_ranks_v1_node:map(Fun, Root)}.

-if(?OTP_RELEASE >= 27).
-doc "Returns a new empty tree.".
-endif.
-spec new() -> tree(_, _).
new() -> #b5_ranks_v1{root = b5_ranks_v1_node:new(), size = 0}.
...
next(Iter) ->
    b5_ranks_v1_node:next(Iter).

%% TODO document
nth(N, #b5_ranks_v1{size = Size, root = Root}) ->
...
size(#b5_ranks_v1{size = Size}) -> Size.

%% TODO document
range(N, Len, #b5_ranks_v1{size = Size, root = Root}) ->
    % TODO negative values of N?
    if
        ...
        true ->
            M = min(Size, N + Len - 1),
            RevLen = M - N + 1,
            b5_ranks_v1_node:range(M, RevLen, Root)
    end.

%% TODO document
rank(Key, #b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:rank(Key, Root).
...
smaller(Key, #b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:smaller(Key, Root).
...
smallest(#b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:smallest(Root).
...
take(Key, #b5_ranks_v1{size = Size, root = Root} = Tree) ->
    [TakenPair | UpdatedRoot] = b5_ranks_v1_node:take(Key, Root),
    UpdatedTree = Tree#b5_ranks_v1{size = Size - 1, root = UpdatedRoot},
...
take_largest(#b5_ranks_v1{size = Size, root = Root} = Tree) ->
    [TakenPair | UpdatedRoot] = b5_ranks_v1_node:take_largest(Root),
    UpdatedTree = Tree#b5_ranks_v1{size = Size - 1, root = UpdatedRoot},
...
take_smallest(#b5_ranks_v1{size = Size, root = Root} = Tree) ->
    [TakenPair | UpdatedRoot] = b5_ranks_v1_node:take_smallest(Root),
    UpdatedTree = Tree#b5_ranks_v1{size = Size - 1, root = UpdatedRoot},
...
to_list(#b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:to_list(Root).
...
update(Key, Value, #b5_ranks_v1{root = Root} = Tree) ->
    Tree#b5_ranks_v1{root = b5_ranks_v1_node:update(Key, eager, Value, Root)}.
...
update_with(Key, Fun, #b5_ranks_v1{root = Root} = Tree) ->
    Tree#b5_ranks_v1{root = b5_ranks_v1_node:update(Key, lazy, Fun, Root)}.
...
update_with(Key, Fun, Init, #b5_ranks_v1{root = Root} = Tree) ->
    try b5_ranks_v1_node:update(Key, lazy, Fun, Root) of
        UpdatedRoot ->
            Tree#b5_ranks_v1{root = UpdatedRoot}
    catch
        error:{badkey, K} when K =:= Key ->
            Tree#b5_ranks_v1{
                root = b5_ranks_v1_node:insert(Key, eager, Init, Root),
                size = Tree#b5_ranks_v1.size + 1
            }
    end.
...
-spec validate(tree(_, _)) ->
    {ok, b5_ranks_v1_node:valid_stats()} | {error, term()}.
validate(#b5_ranks_v1{size = Size, root = Root}) ->
    b5_ranks_v1_node:validate(Size, Root).
...
values(#b5_ranks_v1{root = Root}) ->
    b5_ranks_v1_node:values(Root).
...
want to reuse `b5_ranks_v1_node` functionality while wrapping the tree
...
-spec from_constituent_parts(#{
    root := b5_ranks_v1_node:t(Key, Value), size := non_neg_integer()
}) -> tree(Key, Value).
from_constituent_parts(#{root := Root, size := Size}) when is_integer(Size), Size >= 0 ->
    #b5_ranks_v1{root = Root, size = Size}.
...
-spec to_constituent_parts(tree(Key, Value) | term()) ->
    {ok, #{root := b5_ranks_v1_node:t(Key, Value), size := non_neg_integer()}} | error.
to_constituent_parts(#b5_ranks_v1{root = Root, size = Size}) when is_integer(Size), Size >= 0 ->
