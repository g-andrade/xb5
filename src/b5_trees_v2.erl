...
enter(Key, Value, #b5_trees_v2{size = Size, root = Root} = Tree) ->
    try b5_trees_v2_node:update(Key, eager, Value, Root) of
        UpdatedRoot ->
...
update_with(Key, Fun, Init, #b5_trees_v2{root = Root} = Tree) ->
    try b5_trees_v2_node:update(Key, lazy, Fun, Root) of
        UpdatedRoot ->
            Tree#b5_trees_v2{root = UpdatedRoot}
    catch
        error:{badkey, K} when K =:= Key ->
...
-spec validate(tree(_, _)) ->
    {ok, b5_trees_v2_node:valid_stats()} | {error, term()}.
validate(#b5_trees_v2{size = Size, root = Root}) ->
    b5_trees_v2_node:validate(Size, Root).
