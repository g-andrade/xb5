...
% 3 elements
-define(LEAF1(K1, V1), {leaf1, K1, V1}).
-define(LEAF1_MATCH(K1, V1), {_, K1, V1}).
-define(LEAF1_MATCH_ALL, {_, K1, V1}).

% 2 elements
-define(INTERNAL0(C1), {internal0, C1}).
-define(INTERNAL0_MATCH_ALL, {_, C1}).
...
% improper list
-define(MID_MERGED(Direction, MergedNode), [Direction | MergedNode]).

% 4 elements
-define(MID_ROTATED_FROM_LEFT(UpKey, UpValue, UpdatedLeft, UpdatedNode),
...
-define(new_INTERNAL3(K1, K2, K3, V1, V2, V3, C1, C2, C3, C4),
    ?CHECK_NODE(?INTERNAL3(K1, K2, K3, V1, V2, V3, C1, C2, C3, C4))
).
...
-define(new_LEAF3(K1, K2, K3, V1, V2, V3), ?CHECK_NODE(?LEAF3(K1, K2, K3, V1, V2, V3))).
...
-spec structural_stats(t(_, _)) -> b4_structural_stats:t().
structural_stats(Root) ->
    Acc = b4_structural_stats:new(),

    case Root of
        ?LEAF0 ->
            b4_structural_stats:return(Acc);
        %
        _ ->
            Height = 1,
            Acc2 = structural_stats_recur(Root, Acc, Height),
            b4_structural_stats:return(Acc2)
    end.
...
-spec split_internal(
    ...
    C :: nonempty_node(K, V).
-compile({inline, split_internal/13}).
split_internal(
    K1,
    K2,
    K3,
    ...
    SplitR = ?new_INTERNAL1(K4, V4, C4, C5),
...
keys_recur(Node, Acc) ->
    case Node of
        ...
        %
        ?INTERNAL1_MATCH(K1, _, C1, C2) ->
            keys_recur(C1, [K1 | keys_recur(C2, Acc)])
    end.
...
structural_stats_recur(Node, Acc, Height) ->
    case Node of
        ?LEAF3_MATCH(_, _, _, _, _, _) ->
            Acc2 = b4_structural_stats:inc_count(leaf3, Acc),
            b4_structural_stats:set_height(Height, Acc2);
        %
        ?LEAF2_MATCH(_, _, _, _) ->
            Acc2 = b4_structural_stats:inc_count(leaf2, Acc),
            b4_structural_stats:set_height(Height, Acc2);
        %
        ?LEAF1_MATCH(_, _) ->
            Acc2 = b4_structural_stats:inc_count(leaf1, Acc),
            b4_structural_stats:set_height(Height, Acc2);
        %
        ?INTERNAL3_MATCH(_, _, _, _, _, _, C1, C2, C3, C4) ->
            Acc2 = b4_structural_stats:inc_count(internal3, Acc),
            IncHeight = Height + 1,
            Acc3 = structural_stats_recur(C1, Acc2, IncHeight),
            Acc4 = structural_stats_recur(C2, Acc3, IncHeight),
            Acc5 = structural_stats_recur(C3, Acc4, IncHeight),
            _Acc6 = structural_stats_recur(C4, Acc5, IncHeight);
        %
        ?INTERNAL2_MATCH(_, _, _, _, C1, C2, C3) ->
            Acc2 = b4_structural_stats:inc_count(internal2, Acc),
            IncHeight = Height + 1,
            Acc3 = structural_stats_recur(C1, Acc2, IncHeight),
            Acc4 = structural_stats_recur(C2, Acc3, IncHeight),
            _Acc5 = structural_stats_recur(C3, Acc4, IncHeight);
        %
        ?INTERNAL1_MATCH(_, _, C1, C2) ->
            IncHeight = Height + 1,
            Acc2 = b4_structural_stats:inc_count(internal1, Acc),
            Acc3 = structural_stats_recur(C1, Acc2, IncHeight),
            _Acc4 = structural_stats_recur(C2, Acc3, IncHeight)
    end.
...
values_recur(Node, Acc) ->
    case Node of
        ...
        ?INTERNAL1_MATCH(_, V1, C1, C2) ->
            values_recur(C1, [V1 | values_recur(C2, Acc)])
    end.
...
ins_rebalance_into_left_internal_maybe(
    ?INTERNAL3_MATCH_ALL, Pos, SplitK, SplitV, SplitL, SplitR, ParentK, ParentV, Left
) ->
    case Pos of
        1 ->
...
-compile({inline, ins_rebalance_into_left_internal/16}).
ins_rebalance_into_left_internal(
    K1,
    K2,
    K3,
...
ins_rebalance_into_left_leaf(?LEAF3_MATCH_ALL = Node, Pos, NewKey, NewValue, UpdatedLeft) ->
    case Pos of
        1 ->
            UpKey = NewKey,
            UpValue = NewValue,
        ...
        2 ->
            UpKey = K1,
            UpValue = V1,
            UpdatedNode = ?new_LEAF3(NewKey, K2, K3, NewValue, V2, V3),
            {UpKey, UpValue, UpdatedLeft, UpdatedNode};
        %
        3 ->
            UpKey = K1,
            UpValue = V1,
            UpdatedNode = ?new_LEAF3(K2, NewKey, K3, V2, NewValue, V3),
            {UpKey, UpValue, UpdatedLeft, UpdatedNode};
        %
        4 ->
            UpKey = K1,
            UpValue = V1,
            UpdatedNode = ?new_LEAF3(K2, K3, NewKey, V2, V3, NewValue),
            {UpKey, UpValue, UpdatedLeft, UpdatedNode}
    end.

-compile({inline, ins_rebalance_split_leaf/4}).
ins_rebalance_split_leaf(?LEAF3_MATCH_ALL, Pos, NewKey, NewValue) ->
    case Pos of
        1 ->
            split_leaf(
                NewKey,
...
ins_rebalance_into_right_internal_maybe(
    ?INTERNAL3_MATCH_ALL, Pos, SplitK, SplitV, SplitL, SplitR, ParentK, ParentV, Right
) ->
    case Pos of
        1 ->
...
-compile({inline, ins_rebalance_into_right_internal/16}).
ins_rebalance_into_right_internal(
    K1,
    K2,
    K3,
...
ins_rebalance_into_right_leaf(?LEAF3_MATCH_ALL = Node, Pos, NewKey, NewValue, UpdatedRight) ->
    case Pos of
        1 ->
            UpKey = K3,
            UpValue = V3,
            UpdatedNode = ?new_LEAF3(NewKey, K1, K2, NewValue, V1, V2),
            {UpKey, UpValue, UpdatedNode, UpdatedRight};
        %
        2 ->
            UpKey = K3,
            UpValue = V3,
            UpdatedNode = ?new_LEAF3(K1, NewKey, K2, V1, NewValue, V2),
            {UpKey, UpValue, UpdatedNode, UpdatedRight};
        %
        3 ->
            UpKey = K3,
            UpValue = V3,
            UpdatedNode = ?new_LEAF3(K1, K2, NewKey, V1, V2, NewValue),
            {UpKey, UpValue, UpdatedNode, UpdatedRight};
        %
        4 ->
            UpKey = NewKey,
...
% -compile({inline, del_rebalance_internal_from_right_sibling/4}).
del_rebalance_INTERNAL1_from_right_sibling(
    CKey,
    CValue,
    CLeft,
...
del_rebalance_LEAF1_from_right_sibling(CKey, CValue, RParentK, RParentV, Right) ->
    case Right of
        ?LEAF1_MATCH(RK1, RV1) ->
...
%-compile({inline, del_rebalance_leaf_from_right_sibling/5}).
del_rebalance_LEAF0_from_right_sibling(RParentK, RParentV, Right) ->
    case Right of
        ?LEAF1_MATCH_ALL ->
            MergedNode = ?new_LEAF2(
                RParentK,
                K1,
                %
                RParentV,
        ...
        ?LEAF3_MATCH_ALL ->
            ...
            UpdatedNode = ?new_LEAF1(RParentK, RParentV),
...
del_rebalance_LEAF0_from_left_sibling(
    ...
    case Left of
        ...
        ?LEAF3_MATCH_ALL ->
            ...
            UpdatedNode = ?new_LEAF1(LParentK, LParentV),
...
%-compile({inline, del_rebalance_INTERNAL1_from_either_sibling/10}).
del_rebalance_INTERNAL1_from_either_sibling(
    CKey,
    CValue,
    CLeft,
...
%-compile({inline, del_rebalance_LEAF1_from_either_sibling/8}).
del_rebalance_LEAF1_from_either_sibling(
    CKey,
    CValue,
    %
