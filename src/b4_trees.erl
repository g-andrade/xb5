...
-record(b4_trees, {size, root}).

-opaque tree(Key, Value) :: #b4_trees{
    size :: non_neg_integer(),
    root :: b4_trees_node:t(Key, Value)
}.
-export_type([tree/2]).

%%%%%%%

-type iter(Key, Value) :: b4_trees_node:iter(Key, Value).
-export_type([iter/2]).
...
delete(Key, #b4_trees{root = Root, size = Size} = Tree) ->
    Tree#b4_trees{
        root = b4_trees_node:delete(Key, Root),
        size = Size - 1
    }.

delete_any(Key, #b4_trees{root = Root, size = Size} = Tree) ->
    case b4_trees_node:is_defined(Key, Root) of
        true ->
            Tree#b4_trees{
                root = b4_trees_node:delete(Key, Root),
                size = Size - 1
...
enter(Key, Value, #b4_trees{size = Size, root = Root} = Tree) ->
    case b4_trees_node:is_defined(Key, Root) of
        true ->
            UpdatedRoot = b4_trees_node:update(Key, eager, Value, Root),
            Tree#b4_trees{root = UpdatedRoot};
        %
        false ->
            Tree#b4_trees{
                root = b4_trees_node:insert(Key, eager, Value, Root),
                size = Size + 1
            }
    end.
...
foldl(Fun, Acc0, #b4_trees{root = Root}) ->
    b4_trees_node:foldl(Fun, Acc0, Root).
...
-spec foldr(fun((Key, Value, Acc1) -> Acc2), Acc0, tree(Key, Value)) -> AccN when
    AccN :: Acc2, Acc2 :: Acc1, Acc1 :: Acc0.
foldr(Fun, Acc0, #b4_trees{root = Root}) ->
    b4_trees_node:foldr(Fun, Acc0, Root).
...
get(Key, #b4_trees{root = Root}) ->
    b4_trees_node:get(Key, Root).
...
insert(Key, Value, #b4_trees{size = Size, root = Root} = Tree) ->
    Tree#b4_trees{
        size = Size + 1,
        root = b4_trees_node:insert(Key, eager, Value, Root)
    }.
...
insert_with(Key, Fun, #b4_trees{size = Size, root = Root} = Tree) ->
    Tree#b4_trees{
        size = Size + 1,
        root = b4_trees_node:insert(Key, lazy, Fun, Root)
    }.
...
is_defined(Key, #b4_trees{root = Root}) ->
    b4_trees_node:is_defined(Key, Root).

-if(?OTP_RELEASE >= 27).
-doc "Returns `true` if `Tree` is an empty tree, otherwise `false`.".
-endif.
-spec is_empty(tree(_, _)) -> boolean().
is_empty(#b4_trees{size = Size}) ->
    Size =:= 0.
...
-spec iterator(Tree) -> Iter when
    Tree :: tree(Key, Value),
    Iter :: b4_trees_node:iter(Key, Value).
...
iterator(#b4_trees{root = Root}, Order) ->
    b4_trees_node:iterator(Root, Order).
...
iterator_from(Key, #b4_trees{root = Root}, Order) ->
    b4_trees_node:iterator_from(Key, Root, Order).

-if(?OTP_RELEASE >= 27).
-doc "Returns the keys in `Tree` as an ordered list.".
-endif.
-spec keys(Tree) -> [Key] when Tree :: tree(Key, _).
keys(#b4_trees{root = Root}) ->
    b4_trees_node:keys(Root).
...
larger(Key, #b4_trees{root = Root}) ->
    b4_trees_node:larger(Key, Root).
...
largest(#b4_trees{root = Root}) ->
    b4_trees_node:largest(Root).
...
lookup(Key, #b4_trees{root = Root}) ->
    b4_trees_node:lookup(Key, Root).
...
map(Fun, #b4_trees{root = Root} = Tree) ->
    Tree#b4_trees{root = b4_trees_node:map(Fun, Root)}.

-if(?OTP_RELEASE >= 27).
-doc "Returns a new empty tree.".
-endif.
-spec new() -> tree(_, _).
new() -> #b4_trees{root = b4_trees_node:new(), size = 0}.
...
smaller(Key, #b4_trees{root = Root}) ->
    b4_trees_node:smaller(Key, Root).
...
smallest(#b4_trees{root = Root}) ->
    b4_trees_node:smallest(Root).

% TODO document
structural_stats(#b4_trees{root = Root}) ->
    b4_trees_node:structural_stats(Root).
...
take(Key, #b4_trees{size = Size, root = Root} = Tree) ->
    [TakenPair | UpdatedRoot] = b4_trees_node:take(Key, Root),
    UpdatedTree = Tree#b4_trees{size = Size - 1, root = UpdatedRoot},
...
take_any(Key, #b4_trees{size = Size, root = Root} = Tree) ->
    case b4_trees_node:is_defined(Key, Root) of
        true ->
            [TakenPair | UpdatedRoot] = b4_trees_node:take(Key, Root),
            UpdatedTree = Tree#b4_trees{size = Size - 1, root = UpdatedRoot},
...
take_largest(#b4_trees{size = Size, root = Root} = Tree) ->
    [TakenPair | UpdatedRoot] = b4_trees_node:take_largest(Root),
    UpdatedTree = Tree#b4_trees{size = Size - 1, root = UpdatedRoot},
...
take_smallest(#b4_trees{size = Size, root = Root} = Tree) ->
    [TakenPair | UpdatedRoot] = b4_trees_node:take_smallest(Root),
    UpdatedTree = Tree#b4_trees{size = Size - 1, root = UpdatedRoot},
...
to_list(#b4_trees{root = Root}) ->
    b4_trees_node:to_list(Root).
...
update(Key, Value, #b4_trees{root = Root} = Tree) ->
    Tree#b4_trees{root = b4_trees_node:update(Key, eager, Value, Root)}.
...
update_with(Key, Fun, #b4_trees{root = Root} = Tree) ->
    Tree#b4_trees{root = b4_trees_node:update(Key, lazy, Fun, Root)}.
...
update_with(Key, Fun, Init, #b4_trees{root = Root} = Tree) ->
    case b4_trees_node:lookup(Key, Root) of
        {value, PrevValue} ->
            UpdatedValue = Fun(PrevValue),
            UpdatedRoot = b4_trees_node:update(Key, eager, UpdatedValue, Root),
            Tree#b4_trees{root = UpdatedRoot};
        %
        none ->
            Tree#b4_trees{
                root = b4_trees_node:insert(Key, eager, Init, Root),
                size = Tree#b4_trees.size + 1
            }
    end.
...
values(#b4_trees{root = Root}) ->
    b4_trees_node:values(Root).
...
want to reuse `b4_trees_node` functionality while wrapping the tree
...
-spec from_constituent_parts(#{
    root := b4_trees_node:t(Key, Value), size := non_neg_integer()
}) -> tree(Key, Value).
from_constituent_parts(#{root := Root, size := Size}) when is_integer(Size), Size >= 0 ->
    #b4_trees{root = Root, size = Size}.
...
-spec to_constituent_parts(tree(Key, Value) | term()) ->
    {ok, #{root := b4_trees_node:t(Key, Value), size := non_neg_integer()}} | error.
to_constituent_parts(#b4_trees{root = Root, size = Size}) when is_integer(Size), Size >= 0 ->
