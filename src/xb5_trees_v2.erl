...
-opaque tree(Key, Value) :: #xb5_tree{
    ...
    root :: xb5_trees_v2_node:t(Key, Value)
}.
-export_type([tree/2]).
...
-opaque iter(Key, Value) :: xb5_trees_v2_node:iter(Key, Value).
-export_type([iter/2]).
...
```erlang
> T = xb5_trees_v2:from_list([{3, c}, {1, a}, {2, b}]).
> xb5_trees_v2:to_list(xb5_trees_v2:balance(T)).
[{1, a}, {2, b}, {3, c}]
```
""".
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> xb5_trees_v2:to_list(xb5_trees_v2:delete(2, T)).
[{1, a}, {3, c}]
```
""".
...
delete(Key, #xb5_tree{root = Root, size = Size} = Tree) ->
    case xb5_trees_v2_node:delete_att(Key, Root) of
        badkey ->
            error_badkey(Key);
        %
        UpdatedRoot ->
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> xb5_trees_v2:to_list(xb5_trees_v2:delete_any(2, T)).
[{1, a}, {3, c}]
> xb5_trees_v2:to_list(xb5_trees_v2:delete_any(42, T)).
[{1, a}, {2, b}, {3, c}]
```
""".
...
```erlang
> T0 = xb5_trees_v2:new().
> T1 = xb5_trees_v2:enter(1, a, T0).
> xb5_trees_v2:to_list(T1).
[{1, a}]
> T2 = xb5_trees_v2:enter(1, z, T1).
> xb5_trees_v2:to_list(T2).
[{1, z}]
> T3 = xb5_trees_v2:enter(2, b, T2).
> xb5_trees_v2:to_list(T3).
[{1, z}, {2, b}]
```
""".
...
enter(Key, Value, #xb5_tree{size = Size, root = Root} = Tree) ->
    case xb5_trees_v2_node:insert_att(Key, eager, Value, Root) of
        key_exists ->
            UpdatedRoot = xb5_trees_v2_node:update_att(Key, eager, Value, Root),
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> xb5_trees_v2:foldl(fun(K, V, Acc) -> [{K, V} | Acc] end, [], T).
[{3, c}, {2, b}, {1, a}]
```
""".
...
foldl(Fun, Acc0, #xb5_tree{root = Root}) ->
    xb5_trees_v2_node:foldl(Fun, Acc0, Root).
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> xb5_trees_v2:foldr(fun(K, V, Acc) -> [{K, V} | Acc] end, [], T).
[{1, a}, {2, b}, {3, c}]
```
""".
...
foldr(Fun, Acc0, #xb5_tree{root = Root}) ->
    xb5_trees_v2_node:foldr(Fun, Acc0, Root).
...
```erlang
> xb5_trees_v2:to_list(xb5_trees_v2:from_list([{3, c}, {1, a}, {2, b}])).
[{1, a}, {2, b}, {3, c}]
> xb5_trees_v2:to_list(xb5_trees_v2:from_list([{1, a}, {1, z}])).
[{1, z}]
> xb5_trees_v2:to_list(xb5_trees_v2:from_list([])).
[]
```
""".
...
```erlang
> xb5_trees_v2:to_list(xb5_trees_v2:from_orddict([{1, a}, {2, b}])).
[{1, a}, {2, b}]
```
""".
...
from_orddict(Orddict) ->
    S = length(Orddict),
    Root = xb5_trees_v2_node:from_orddict(Orddict, S),
...
get(Key, #xb5_tree{root = Root}) ->
    xb5_trees_v2_node:get(Key, Root).
...
```erlang
> T0 = xb5_trees_v2:new().
> T1 = xb5_trees_v2:insert(1, a, T0).
> xb5_trees_v2:to_list(T1).
[{1, a}]
> T2 = xb5_trees_v2:insert(2, b, T1).
> xb5_trees_v2:to_list(T2).
[{1, a}, {2, b}]
```
""".
...
```erlang
> T0 = xb5_trees_v2:new().
> T1 = xb5_trees_v2:insert_with(1, fun() -> a end, T0).
> xb5_trees_v2:to_list(T1).
[{1, a}]
```
""".
...
insert_with(Key, Fun, #xb5_tree{size = Size, root = Root} = Tree) ->
    case xb5_trees_v2_node:insert_att(Key, lazy, Fun, Root) of
        key_exists ->
            error_key_exists(Key);
        %
        UpdatedRoot ->
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> xb5_trees_v2:is_defined(2, T).
true
> xb5_trees_v2:is_defined(42, T).
false
```
""".
...
is_defined(Key, #xb5_tree{root = Root}) ->
    xb5_trees_v2_node:is_defined(Key, Root).
...
```erlang
> xb5_trees_v2:is_empty(xb5_trees_v2:new()).
true
> xb5_trees_v2:is_empty(xb5_trees_v2:from_list([{1, a}])).
false
```
""".
...
```erlang
> T = xb5_trees_v2:from_list([{3, c}, {1, a}, {2, b}]).
> I = xb5_trees_v2:iterator(T).
> {1, a, I2} = xb5_trees_v2:next(I).
> {2, b, I3} = xb5_trees_v2:next(I2).
> {3, c, I4} = xb5_trees_v2:next(I3).
> xb5_trees_v2:next(I4).
none
```
""".
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> I = xb5_trees_v2:iterator(T, reversed).
> {3, c, I2} = xb5_trees_v2:next(I).
> {2, b, I3} = xb5_trees_v2:next(I2).
> {1, a, I4} = xb5_trees_v2:next(I3).
> xb5_trees_v2:next(I4).
none
```
""".
...
iterator(#xb5_tree{root = Root}, Order) ->
    xb5_trees_v2_node:iterator(Root, Order).
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}, {4, d}, {5, e}]).
> I = xb5_trees_v2:iterator_from(3, T).
> {3, c, I2} = xb5_trees_v2:next(I).
> {4, d, I3} = xb5_trees_v2:next(I2).
> {5, e, I4} = xb5_trees_v2:next(I3).
> xb5_trees_v2:next(I4).
none
```
""".
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}, {4, d}, {5, e}]).
> I = xb5_trees_v2:iterator_from(3, T, reversed).
> {3, c, I2} = xb5_trees_v2:next(I).
> {2, b, I3} = xb5_trees_v2:next(I2).
> {1, a, I4} = xb5_trees_v2:next(I3).
> xb5_trees_v2:next(I4).
...
iterator_from(Key, #xb5_tree{root = Root}, Order) ->
    xb5_trees_v2_node:iterator_from(Key, Root, Order).
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {3, c}, {5, e}]).
> xb5_trees_v2:larger(2, T).
{3, c}
> xb5_trees_v2:larger(5, T).
none
```
""".
...
larger(Key, #xb5_tree{root = Root}) ->
    xb5_trees_v2_node:larger(Key, Root).
...
```erlang
> xb5_trees_v2:largest(xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}])).
{3, c}
```
""".
...
largest(#xb5_tree{size = Size, root = Root}) when Size =/= 0 ->
    xb5_trees_v2_node:largest(Root);
largest(#xb5_tree{}) ->
    error_empty_tree().
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> xb5_trees_v2:lookup(2, T).
{value, b}
> xb5_trees_v2:lookup(42, T).
none
```
""".
...
lookup(Key, #xb5_tree{root = Root}) ->
    xb5_trees_v2_node:lookup(Key, Root).
...
```erlang
> T = xb5_trees_v2:from_list([{1, 10}, {2, 20}, {3, 30}]).
> xb5_trees_v2:to_list(xb5_trees_v2:map(fun(_K, V) -> V * 2 end, T)).
[{1, 20}, {2, 40}, {3, 60}]
```
""".
...
map(Fun, #xb5_tree{root = Root} = Tree) ->
    Tree#xb5_tree{root = xb5_trees_v2_node:map(Fun, Root)}.
...
> xb5_trees_v2:is_empty(xb5_trees_v2:new()).
true
> xb5_trees_v2:size(xb5_trees_v2:new()).
0
```
""".
-spec new() -> tree().

new() -> #xb5_tree{root = xb5_trees_v2_node:new(), size = 0}.
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}]).
> I = xb5_trees_v2:iterator(T).
> {1, a, I2} = xb5_trees_v2:next(I).
> {2, b, I3} = xb5_trees_v2:next(I2).
> xb5_trees_v2:next(I3).
none
```
""".
...
> xb5_trees_v2:size(xb5_trees_v2:new()).
0
> xb5_trees_v2:size(xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}])).
3
```
""".
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {3, c}, {5, e}]).
> xb5_trees_v2:smaller(4, T).
{3, c}
> xb5_trees_v2:smaller(1, T).
none
```
""".
...
smaller(Key, #xb5_tree{root = Root}) ->
    xb5_trees_v2_node:smaller(Key, Root).
...
```erlang
> xb5_trees_v2:smallest(xb5_trees_v2:from_list([{3, c}, {1, a}, {2, b}])).
{1, a}
```
""".
...
smallest(#xb5_tree{size = Size, root = Root}) when Size =/= 0 ->
    xb5_trees_v2_node:smallest(Root);
smallest(#xb5_tree{}) ->
    error_empty_tree().
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> Stats = xb5_trees_v2:structural_stats(T).
...
structural_stats(#xb5_tree{root = Root}) ->
    xb5_trees_v2_node:structural_stats(Root).
...
take(Key, #xb5_tree{size = Size, root = Root} = Tree) ->
    case xb5_trees_v2_node:take_att(Key, Root) of
        badkey ->
            error_badkey(Key);
        %
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> {b, T2} = xb5_trees_v2:take_any(2, T).
> xb5_trees_v2:to_list(T2).
[{1, a}, {3, c}]
> xb5_trees_v2:take_any(42, T).
error
```
""".
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> {3, c, T2} = xb5_trees_v2:take_largest(T).
> xb5_trees_v2:to_list(T2).
[{1, a}, {2, b}]
```
""".
...
take_largest(#xb5_tree{size = Size, root = Root} = Tree) when Size =/= 0 ->
    [TakenPair | UpdatedRoot] = xb5_trees_v2_node:take_largest(Root),
    UpdatedTree = Tree#xb5_tree{size = Size - 1, root = UpdatedRoot},
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> {1, a, T2} = xb5_trees_v2:take_smallest(T).
> xb5_trees_v2:to_list(T2).
[{2, b}, {3, c}]
```
""".
...
take_smallest(#xb5_tree{size = Size, root = Root} = Tree) when Size =/= 0 ->
    [TakenPair | UpdatedRoot] = xb5_trees_v2_node:take_smallest(Root),
    UpdatedTree = Tree#xb5_tree{size = Size - 1, root = UpdatedRoot},
...
> xb5_trees_v2:to_list(xb5_trees_v2:from_list([{3, c}, {1, a}, {2, b}])).
[{1, a}, {2, b}, {3, c}]
> xb5_trees_v2:to_list(xb5_trees_v2:new()).
[]
```
""".
...
to_list(#xb5_tree{root = Root}) ->
    xb5_trees_v2_node:to_list(Root).
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> {ok, Unwrapped} = xb5_trees_v2:unwrap(T).
> maps:get(size, Unwrapped).
3
> {error, _} = xb5_trees_v2:unwrap(not_a_tree).
```
""".
...
unwrap(#xb5_tree{size = Size, root = Root} = Term) when is_integer(Size), Size >= 0 ->
    try xb5_trees_v2_node:structural_stats(Root) of
        Stats ->
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> xb5_trees_v2:to_list(xb5_trees_v2:update(2, z, T)).
[{1, a}, {2, z}, {3, c}]
```
""".
...
update(Key, Value, #xb5_tree{root = Root} = Tree) ->
    case xb5_trees_v2_node:update_att(Key, eager, Value, Root) of
        badkey ->
            error_badkey(Key);
        %
        UpdatedRoot ->
...
```erlang
> T = xb5_trees_v2:from_list([{1, 10}, {2, 20}, {3, 30}]).
> xb5_trees_v2:to_list(xb5_trees_v2:update_with(2, fun(V) -> V + 1 end, T)).
[{1, 10}, {2, 21}, {3, 30}]
```
""".
...
update_with(Key, Fun, #xb5_tree{root = Root} = Tree) ->
    case xb5_trees_v2_node:update_att(Key, lazy, Fun, Root) of
        badkey ->
            error_badkey(Key);
        %
        UpdatedRoot ->
...
```erlang
> T = xb5_trees_v2:from_list([{1, 10}, {2, 20}]).
> T2 = xb5_trees_v2:update_with(2, fun(V) -> V + 1 end, 0, T).
> xb5_trees_v2:to_list(T2).
[{1, 10}, {2, 21}]
> T3 = xb5_trees_v2:update_with(3, fun(V) -> V + 1 end, 0, T).
> xb5_trees_v2:to_list(T3).
[{1, 10}, {2, 20}, {3, 0}]
```
""".
...
update_with(Key, Fun, Init, #xb5_tree{root = Root} = Tree) ->
    case xb5_trees_v2_node:update_att(Key, lazy, Fun, Root) of
        badkey ->
            Tree#xb5_tree{
                root = xb5_trees_v2_node:insert_att(Key, eager, Init, Root),
...
```erlang
> T = xb5_trees_v2:from_list([{1, a}, {2, b}, {3, c}]).
> {ok, U} = xb5_trees_v2:unwrap(T).
> T2 = xb5_trees_v2:wrap(U).
> xb5_trees_v2:to_list(T2).
[{1, a}, {2, b}, {3, c}]
```
""".
