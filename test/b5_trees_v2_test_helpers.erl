%% @doc Helper functions for B5 trees testing, ported from Elixir test suite
-module(b5_trees_v2_test_helpers).
...
for_each_tree(TreeSizes, TestFun) ->
    lists:foreach(
        fun(TreeSize) ->
            ...
            Tree = b5_trees_v2:from_list(maps:to_list(ExistentMap)),
            case b5_trees_v2:validate(Tree) of
                {ok, _} -> ok;
