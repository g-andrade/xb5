...
test_construction(_Config) ->
    foreach_tested_size(
        fun(Size, RefElements) ->
            Set = xb5_sets_v2:from_list(RefElements),
            ?assertListsCanonEqual(RefElements, xb5_sets_v2:to_list(Set)),
            ?assertEqual(Size, xb5_sets_v2:size(Set)),
            ?assertEqual(Size =:= 0, xb5_sets_v2:is_empty(Set)),

            ?assertEqual(RefElements, xb5_sets_v2:to_list(new_set_from_each_inserted(RefElements))),

            ?assertEqual(RefElements, xb5_sets_v2:to_list(xb5_sets_v2:from_ordset(RefElements))),

            case RefElements of
                [] ->
                    ?assertEqual(Set, xb5_sets_v2:empty());
                %
                [SingleElement] ->
                    ?assertEqual(Set, xb5_sets_v2:singleton(SingleElement));
                %
                _ ->
                    ok
            end
    ...
    foreach_test_set(
        fun(Size, RefElements, Set) ->
            foreach_existing_element(
                fun(Element) ->
                    ?assertEqual(true, xb5_sets_v2:is_member(Element, Set)),
                    ?assertEqual(true, xb5_sets_v2:is_element(Element, Set))
                end,
                RefElements,
                Size
            ),
            ...
            foreach_non_existent_element(
                fun(Element) ->
                    ?assertEqual(false, xb5_sets_v2:is_member(Element, Set)),
                    ?assertEqual(false, xb5_sets_v2:is_element(Element, Set))
                end,
                RefElements,
                100
            )
...
test_add(_Config) ->
    foreach_test_set(
        fun(Size, RefElements, Set) ->
            foreach_existing_element(
                fun(Element) ->
                    Set2 = xb5_sets_v2:add(Element, Set),
                    ?assertEqual(Size, xb5_sets_v2:size(Set2)),
                    ?assertListsCanonEqual(
                        RefElements,
                        xb5_sets_v2:to_list(Set2)
                    ),

                    ?assertEqual(Set2, xb5_sets_v2:add_element(Element, Set))
                end,
                RefElements,
                min(50, Size)
            ),
            ...
            foreach_non_existent_element(
                fun(Element) ->
                    Set2 = xb5_sets_v2:add(Element, Set),
                    ?assertEqual(Size + 1, xb5_sets_v2:size(Set2)),
                    ?assertListsCanonEqual(
...
test_insert(_Config) ->
    foreach_test_set(
        fun(Size, RefElements, Set) ->
            foreach_existing_element(
                fun(Element) ->
                    ?assertError({key_exists, Element}, xb5_sets_v2:insert(Element, Set))
                end,
                RefElements,
                min(50, Size)
            ),
            ...
            foreach_non_existent_element(
                fun(Element) ->
                    Set2 = xb5_sets_v2:insert(Element, Set),
                    ?assertEqual(Size + 1, xb5_sets_v2:size(Set2)),
                    ?assertListsCanonEqual(
                        add_to_sorted_list(Element, RefElements),
                        xb5_sets_v2:to_list(Set2)
...
test_delete_sequential(_Config) ->
    foreach_test_set(
        fun(_Size, RefElements, Set) ->
            ...
            {SetN, []} =
                lists:foldl(
                    fun(Element, {Set1, RemainingElements1}) ->
                        ...
                        Set2 = xb5_sets_v2:delete(Element, Set1),
                        RemainingElements2 = remove_from_sorted_list(Element, RemainingElements1),
                        ?assertListsCanonEqual(RemainingElements2, xb5_sets_v2:to_list(Set2)),
                        ?assertEqual(length(RemainingElements2), xb5_sets_v2:size(Set2)),
                        ?assertEqual(RemainingElements2 =:= [], xb5_sets_v2:is_empty(Set2)),

                        ?assertEqual(Set2, xb5_sets_v2:delete_any(Element, Set1)),
                        ?assertEqual(Set2, xb5_sets_v2:del_element(Element, Set1)),
            ...
            ?assertEqual([], xb5_sets_v2:to_list(SetN)),
            ?assertEqual(0, xb5_sets_v2:size(SetN)),
            ?assertEqual(true, xb5_sets_v2:is_empty(SetN)),
            ...
            {SetN, []} =
                lists:foldl(
                    fun(Element, {Set1, RemainingElements1}) ->
                        ...
                        logger:notice("Deleting ~p from set ~p", [Element, Set1]),
...
test_smallest(_Config) ->
    foreach_test_set(
        fun
            (0, _RefElements, Set) ->
                ?assertError(empty_set, xb5_sets_v2:smallest(Set));
            %
            (_Size, RefElements, Set) ->
                ?assert(xb5_sets_v2:smallest(Set) == hd(RefElements))
        end
    ).
...
test_largest(_Config) ->
    foreach_test_set(
        fun
            (0, _RefElements, Set) ->
                ?assertError(empty_set, xb5_sets_v2:largest(Set));
            %
            (_Size, RefElements, Set) ->
                ?assert(xb5_sets_v2:largest(Set) == lists:last(RefElements))
        end
    ).
...
test_intersection(_Config) ->
    ?assertError(function_clause, xb5_sets_v2:intersection(xb5_utils:dialyzer_opaque_term([]))),
...
test_union(_Config) ->
    ?assertEqual(xb5_sets_v2:new(), xb5_sets_v2:union([])),
...
test_iterator_reversed(_Config) ->
    foreach_test_set(
        fun(_Size, RefElements, Set) ->
            Iter = xb5_sets_v2:iterator(Set, reversed),
...
test_balance(_Config) ->
    foreach_test_set(
        fun(_Size, _RefElements, Set) ->
            ...
            ?assertEqual(Set, xb5_sets_v2:balance(Set))
        end
    ).
...
test_is_set(_Config) ->
    ?assertEqual(false, xb5_sets_v2:is_set(foobar)),
    ?assertEqual(false, xb5_sets_v2:is_set(xb5_trees:new())),
    ?assertEqual(false, xb5_sets_v2:is_set(xb5_bag:new())),

    foreach_test_set(
        fun(_Size, _RefElements, Set) ->
            ?assertEqual(true, xb5_sets_v2:is_set(Set)),

            {xb5_set, Size, Root} = xb5_utils:dialyzer_opaque_term(Set),

            ?assertEqual(true, xb5_sets_v2:is_set({xb5_set, Size, Root})),

            ?assertEqual(false, xb5_sets_v2:is_set({xb5_set, -1, Root})),

            ?assertEqual((Size =:= 0), xb5_sets_v2:is_set({xb5_set, 0, Root})),

            ?assertEqual(false, xb5_sets_v2:is_set({xb5_set, Size, invalid_root})),

            ?assertEqual(false, xb5_sets_v2:is_set({xb5_set, not_an_integer, Root}))
        end
    ).
...
test_rewrap(_Config) ->
    ?assertMatch({error, _}, xb5_sets_v2:unwrap(xb5_bag:new())),
    ?assertMatch({error, _}, xb5_sets_v2:unwrap(xb5_trees:new())),
    ?assertMatch({error, _}, xb5_sets_v2:unwrap({xb5_set, -1, xb5_sets_v2_node:new()})),
    ?assertMatch({error, _}, xb5_sets_v2:unwrap({xb5_set, 2, xb5_sets_v2_node:new()})),
    ?assertMatch({error, _}, xb5_sets_v2:unwrap({xb5_set, 2, make_ref()})),

    foreach_test_set(
        fun(_Size, _RefElements, Col) ->
            {ok, Unwrapped} = xb5_sets_v2:unwrap(Col),
            ?assertEqual(Col, xb5_sets_v2:wrap(Unwrapped))
        end
    ).
...
test_structure_built_from_ordset(_Config) ->
    CondensedStats =
        run_structure_test(
            fun(RefElements) ->
                ?assertEqual(RefElements, lists:usort(RefElements)),
                xb5_sets_v2:from_ordset(RefElements)
            end
        ),
    ...
    CondensedStats =
        run_structure_test(
            fun(RefElements) ->
                ...
                AmountToDelete = xb5_sets_v2:size(Set1) div 2,
                ItemsToDelete = lists:sublist(RefElements, AmountToDelete),
                lists:foldl(fun xb5_sets_v2:delete/2, Set1, ItemsToDelete)
            end,
            [{size_multiplier, 2}]
        ),
...
foreach_test_set(Fun, Opts) ->
    foreach_tested_size(
        fun(Size, RefElements) ->
            ...
            Stats = xb5_sets_v2:structural_stats(Set),
...
new_set_from_each_inserted(List) ->
    ...
    ?assertEqual(0, xb5_sets_v2:size(Set)),
    ?assertEqual(true, xb5_sets_v2:is_empty(Set)),

    new_set_from_each_inserted_recur(List, Set).

new_set_from_each_inserted_recur([Element | Next], Set) ->
    UpdatedSet = xb5_sets_v2:insert(Element, Set),
...
run_construction_repeated_test(Size, [ElementToRepeat | Next], RefElements) ->
    ...
    Set = xb5_sets_v2:from_list(List),

    ?assertEqual(Size, xb5_sets_v2:size(Set)),

    ?assertListsCanonEqual(
        RefElements,
        xb5_sets_v2:to_list(Set)
    ),

    %%%

    SetShuffled = xb5_sets_v2:from_list(list_shuffle(List)),

    ?assertEqual(Size, xb5_sets_v2:size(SetShuffled)),

    ?assertListsCanonEqual(
        RefElements,
        xb5_sets_v2:to_list(SetShuffled)
    ),
...
test_extensive_construction_from_ordset_recur(FirstKey, Acc) when FirstKey > 0 ->
    List = [FirstKey | Acc],
    Set = xb5_sets_v2:from_ordset(List),

    ?assertListsCanonEqual(List, xb5_sets_v2:to_list(Set)),

    _ = xb5_sets_v2:structural_stats(Set),
...
test_delete_non_existing_keys(Set, RemainingElements, Amount) when Amount > 0 ->
    ...
    case lists:any(fun(E) -> E == Element end, RemainingElements) of
        false ->
            ?assertError({badkey, Element}, xb5_sets_v2:delete(Element, Set)),
            ?assertEqual(Set, xb5_sets_v2:delete_any(Element, Set)),

            ?assertEqual(Set, xb5_sets_v2:del_element(Element, Set)),
...
run_smaller(RefElements, Set) ->
    case RefElements of
        [] ->
            Element = new_element(),
            ?assertEqual(none, xb5_sets_v2:smaller(Element, Set));
        %
        [SingleElement] ->
            ?assertEqual(
                none, xb5_sets_v2:smaller(randomly_switch_number_type(SingleElement), Set)
            ),

            LargerElement = element_larger(SingleElement),
            ?assert(xb5_sets_v2:smaller(LargerElement, Set) == {found, SingleElement}),

            SmallerElement = element_smaller(SingleElement),
            ?assertEqual(none, xb5_sets_v2:smaller(SmallerElement, Set));
        %
        [FirstElement | Next] ->
            ?assertEqual(none, xb5_sets_v2:smaller(randomly_switch_number_type(FirstElement), Set)),
...
run_smaller_recur(Expected, [LastElement], Set) ->
    ?assertCanonEqual(
        {found, Expected},
        xb5_sets_v2:smaller(randomly_switch_number_type(LastElement), Set)
    ),

    LargerElement = element_larger(LastElement),
    ?assert(LargerElement > LastElement),
    ?assert(xb5_sets_v2:smaller(LargerElement, Set) == {found, LastElement});
run_smaller_recur(Expected, [Element | Next], Set) ->
    ?assert(xb5_sets_v2:smaller(randomly_switch_number_type(Element), Set) == {found, Expected}),

    case element_in_between(Expected, Element) of
        {found, InBetween} ->
            ?assert(InBetween > Expected),
            ?assert(InBetween < Element),
            ?assert(xb5_sets_v2:smaller(InBetween, Set) == {found, Expected});
        %
        none ->
            ok
    end,
...
run_larger(RefElements, Set) ->
    case lists:reverse(RefElements) of
        [] ->
            Element = new_element(),
            ?assertEqual(none, xb5_sets_v2:larger(Element, Set));
        %
        [SingleElement] ->
            ?assertEqual(none, xb5_sets_v2:larger(SingleElement, Set)),

            LargerElement = element_larger(SingleElement),
            ?assertEqual(none, xb5_sets_v2:larger(LargerElement, Set)),

            SmallerElement = element_smaller(SingleElement),
            ?assert(xb5_sets_v2:larger(SmallerElement, Set) == {found, SingleElement});
        %
        [LastElement | Next] ->
            ?assertEqual(none, xb5_sets_v2:larger(randomly_switch_number_type(LastElement), Set)),
...
run_larger_recur(Expected, [FirstElement], Set) ->
    ?assertCanonEqual(
        {found, Expected},
        xb5_sets_v2:larger(randomly_switch_number_type(FirstElement), Set)
    ),

    SmallerElement = element_smaller(FirstElement),
    ?assert(SmallerElement < FirstElement),
    ?assert(xb5_sets_v2:larger(SmallerElement, Set) == {found, FirstElement});
run_larger_recur(Expected, [Element | Next], Set) ->
    ?assert(xb5_sets_v2:larger(randomly_switch_number_type(Element), Set) == {found, Expected}),

    case element_in_between(Element, Expected) of
        {found, InBetween} ->
            ?assert(InBetween < Expected),
            ?assert(InBetween > Element),
            ?assert(xb5_sets_v2:larger(InBetween, Set) == {found, Expected});
        %
        none ->
            ok
    end,
...
run_take_smallest([Expected | Next], Set) ->
    {Taken, Set2} = xb5_sets_v2:take_smallest(Set),
    ?assert(Taken == Expected),
    ?assertEqual(length(Next), xb5_sets_v2:size(Set2)),
    run_take_smallest(Next, Set2);
run_take_smallest([], Set) ->
    ?assertError(empty_set, xb5_sets_v2:take_smallest(Set)).

run_take_largest([Expected | Next], Set) ->
    {Taken, Set2} = xb5_sets_v2:take_largest(Set),
    ?assert(Taken == Expected),
    ?assertEqual(length(Next), xb5_sets_v2:size(Set2)),
    run_take_largest(Next, Set2);
run_take_largest([], Set) ->
    ?assertError(empty_set, xb5_sets_v2:take_largest(Set)).
...
run_difference_test(Size, RefElements, Set) ->
    SelfDifference = xb5_sets_v2:difference(Set, Set),

    ?assertEqual(0, xb5_sets_v2:size(SelfDifference)),
    ?assertEqual([], xb5_sets_v2:to_list(SelfDifference)),

    %%%%%%%%%%%%%%%

    foreach_second_set(
        fun(RefElements2, Set2) ->
            Difference = xb5_sets_v2:difference(Set, Set2),
            ...
            ?assertEqual(
                ...
                xb5_sets_v2:size(Difference)
            ),

            ?assertListsCanonEqual(
                ordsets:to_list(ExpectedRemainingElements),
                xb5_sets_v2:to_list(Difference)
            ),

            %%

            ?assertEqual(Difference, xb5_sets_v2:subtract(Set, Set2))
        end,
        Size,
        RefElements
    ).
...
foreach_second_set_variants1(Fun, Size, RefElements, Opts) ->
    ...
    lists:foreach(
        fun({Amount2, PercentageInCommon}) ->
            ...
            Set2 = xb5_sets_v2:from_list(maybe_shuffle_elements_for_new_set(RefElements2)),
...
foreach_second_set_variants2(Fun, Size, RefElements) ->
    ...
    lists:foreach(
        fun(RefElements2) ->
            Set2 = xb5_sets_v2:from_list(RefElements2),
            Fun(RefElements2, Set2)
        end,
        List2
    ).
...
run_intersection_test(Size, RefElements, Set) ->
    SelfIntersect = xb5_sets_v2:intersection(Set, Set),

    ?assertEqual(Size, xb5_sets_v2:size(SelfIntersect)),

    ?assertListsCanonEqual(
        RefElements,
        xb5_sets_v2:to_list(SelfIntersect)
    ),
    ...
    foreach_second_set(
        fun(RefElements2, Set2) ->
            Intersection = xb5_sets_v2:intersection(Set, Set2),
            ...
            ?assertEqual(
                ordsets:size(ExpectedRemainingElements),
                xb5_sets_v2:size(Intersection)
            ),

            ?assertListsCanonEqual(
                ordsets:to_list(ExpectedRemainingElements),
                xb5_sets_v2:to_list(Intersection)
            ),
            ...
            ?assertListsCanonEqual(
                ordsets:to_list(ExpectedRemainingElements),
                xb5_sets_v2:to_list(xb5_sets_v2:intersection(Set, Set2))
            )
        end,
        Size,
        RefElements
        ...
        fun(RefElements2, Set2) ->
            foreach_second_set(
                fun(RefElements3, Set3) ->
                    ...
                    Intersection = xb5_sets_v2:intersection(IntersectionInputs),

                    ?assertEqual(length(ExpectedRemainingElements), xb5_sets_v2:size(Intersection)),

                    ?assertListsCanonEqual(
                        ExpectedRemainingElements, xb5_sets_v2:to_list(Intersection)
                    ),

                    %%%

                    Intersection2 = xb5_sets_v2:intersection(list_shuffle(IntersectionInputs)),

                    ?assertEqual(
                        length(ExpectedRemainingElements), xb5_sets_v2:size(Intersection2)
                    ),

                    ?assertListsCanonEqual(
                        ExpectedRemainingElements, xb5_sets_v2:to_list(Intersection2)
                    ),

                    %%%

                    Intersection3 = xb5_sets_v2:intersection(list_shuffle(IntersectionInputs)),

                    ?assertEqual(
                        length(ExpectedRemainingElements), xb5_sets_v2:size(Intersection3)
                    ),

                    ?assertListsCanonEqual(
                        ExpectedRemainingElements, xb5_sets_v2:to_list(Intersection3)
                    )
                end,
                Size,
                RefElements,
...
run_is_disjoint_test(Size, RefElements, Set) ->
    ?assertEqual((Size =:= 0), xb5_sets_v2:is_disjoint(Set, Set)),

    %%%%%%%

    foreach_second_set(
        fun(RefElements2, Set2) ->
            IsDisjoint = xb5_sets_v2:is_disjoint(Set, Set2),
            ...
            ?assertEqual(ExpectedIsDisjoint, xb5_sets_v2:is_disjoint(Set2, Set))
        end,
        Size,
        RefElements
    ).
...
run_is_equal_test(Size, RefElements, Set) ->
    ?assertEqual(true, xb5_sets_v2:is_equal(Set, Set)),

    %%%%%%%

    foreach_second_set(
        fun(RefElements2, Set2) ->
            IsEqual = xb5_sets_v2:is_equal(Set, Set2),
            ...
            ?assertEqual(ExpectedIsEqual, xb5_sets_v2:is_equal(Set2, Set))
        end,
        Size,
        RefElements
    ).
...
run_is_subset_test(Size, RefElements, Set) ->
    ?assertEqual(true, xb5_sets_v2:is_subset(Set, Set)),

    %%%%%%

    foreach_second_set(
        fun(RefElements2, Set2) ->
            IsSubset = xb5_sets_v2:is_subset(Set, Set2),
            ...
            Size1 = xb5_sets_v2:size(Set),
            Size2 = xb5_sets_v2:size(Set2),

            case Size1 =:= Size2 of
                true ->
                    ?assertEqual(IsSubset, xb5_sets_v2:is_subset(Set2, Set));
                _ ->
                    ok
            end
        end,
...
run_union_test(Size, RefElements, Set) ->
    SelfUnion = xb5_sets_v2:union(Set, Set),

    ?assertEqual(Size, xb5_sets_v2:size(SelfUnion)),

    ?assertListsCanonEqual(RefElements, xb5_sets_v2:to_list(SelfUnion)),
    ...
    foreach_second_set(
        fun(RefElements2, Set2) ->
            ...
            Union = xb5_sets_v2:union(Set, Set2),

            ?assertEqual(
                length(ExpectedUnionElements),
                xb5_sets_v2:size(Union)
            ),

            ?assertListsCanonEqual(
                ExpectedUnionElements,
                xb5_sets_v2:to_list(Union)
            ),

            %%%%%%%

            Union2 = xb5_sets_v2:union(Set2, Set),

            ?assertEqual(
                length(ExpectedUnionElements),
                xb5_sets_v2:size(Union2)
            ),

            ?assertListsCanonEqual(
                ExpectedUnionElements,
                xb5_sets_v2:to_list(Union2)
            ),

            %%%%%%%%%

            Union3 = xb5_sets_v2:union([Set, Set2]),

            ?assertEqual(
                length(ExpectedUnionElements),
                xb5_sets_v2:size(Union3)
            ),

            ?assertListsCanonEqual(
                ExpectedUnionElements,
                xb5_sets_v2:to_list(Union3)
            ),

            %%%%%%%%%

            Union4 = xb5_sets_v2:union([Set2, Set]),

            ?assertEqual(
                length(ExpectedUnionElements),
                xb5_sets_v2:size(Union4)
            ),

            ?assertListsCanonEqual(
                ExpectedUnionElements,
                xb5_sets_v2:to_list(Union4)
            ),
            ...
            Union5 = xb5_sets_v2:union(Union5Inputs),

            ?assertEqual(
                length(ExpectedUnionElements),
                xb5_sets_v2:size(Union5)
            ),

            ?assertListsCanonEqual(
                ExpectedUnionElements,
                xb5_sets_v2:to_list(Union5)
            )
        end,
        Size,
        RefElements,
        ...
        fun(RefElements2, Set2) ->
            foreach_second_set(
                fun(RefElements3, Set3) ->
                    ...
                    Union = xb5_sets_v2:union(UnionInputs),

                    ?assertEqual(length(ExpectedUnionElements), xb5_sets_v2:size(Union)),

                    ?assertListsCanonEqual(ExpectedUnionElements, xb5_sets_v2:to_list(Union)),

                    %%%

                    Union2 = xb5_sets_v2:union(list_shuffle(UnionInputs)),

                    ?assertEqual(length(ExpectedUnionElements), xb5_sets_v2:size(Union2)),

                    ?assertListsCanonEqual(ExpectedUnionElements, xb5_sets_v2:to_list(Union2)),

                    %%%

                    Union3 = xb5_sets_v2:union(list_shuffle(UnionInputs)),

                    ?assertEqual(length(ExpectedUnionElements), xb5_sets_v2:size(Union3)),

                    ?assertListsCanonEqual(ExpectedUnionElements, xb5_sets_v2:to_list(Union3))
                end,
                Size,
                RefElements,
                [{max_combos, 4}]
...
new_iterator_from(Elem, Set) ->
    Iter = xb5_sets_v2:iterator_from(Elem, Set),
    ?assertEqual(Iter, xb5_sets_v2:iterator_from(Elem, Set, ordered)),
    Iter.
...
run_iterator_from_reversed(RefElements, Set) ->
    case lists:reverse(RefElements) of
        [] ->
            Iter = xb5_sets_v2:iterator_from(new_element(), Set, reversed),
            ?assertEqual([], iterate(Iter));
        %
        [SingleElement] ->
            Iter = xb5_sets_v2:iterator_from(
                randomly_switch_number_type(SingleElement), Set, reversed
            ),
            ?assertListsCanonEqual([SingleElement], iterate(Iter)),

            SmallerElement = element_smaller(SingleElement),
            Iter2 = xb5_sets_v2:iterator_from(SmallerElement, Set, reversed),
            ?assertEqual([], iterate(Iter2)),

            LargerElement = element_larger(SingleElement),
            Iter3 = xb5_sets_v2:iterator_from(LargerElement, Set, reversed),
            ?assertListsCanonEqual([SingleElement], iterate(Iter3));
        %
        [LastElement | _] = ReverseRefElements ->
            LargerElement = element_larger(LastElement),
            Iter = xb5_sets_v2:iterator_from(LargerElement, Set, reversed),
...
run_iterator_from_reversed_recur([Elem2 | [Elem1 | _] = Tail] = List, Set) ->
    Iter = xb5_sets_v2:iterator_from(Elem2, Set, reversed),
    ?assertListsCanonEqual(List, iterate(Iter)),
    ...
    case element_in_between(Elem1, Elem2) of
        {found, InBetween} ->
            ?assert(InBetween > Elem1),
            ?assert(InBetween < Elem2),
            Iter2 = xb5_sets_v2:iterator_from(InBetween, Set, reversed),
...
run_iterator_from_reversed_first_element(FirstElement, Set) ->
    Iter = xb5_sets_v2:iterator_from(randomly_switch_number_type(FirstElement), Set, reversed),
...
new_iterator(Set) ->
    Iter = xb5_sets_v2:iterator(Set),
    ?assertEqual(Iter, xb5_sets_v2:iterator(Set, ordered)),
    Iter.

iterate(Iter) ->
    case xb5_sets_v2:next(Iter) of
        {Element, Iter2} ->
...
run_filter(Size, RefElements, Set) ->
    ...
    lists:foreach(
        fun(AmountToRemove) ->
            ...
            FilteredSet = xb5_sets_v2:filter(FilterFun, Set),

            ExpectedElementsRemaining = lists:filter(FilterFun, RefElements),
            ?assertListsCanonEqual(ExpectedElementsRemaining, xb5_sets_v2:to_list(FilteredSet)),

            ?assertEqual(length(ExpectedElementsRemaining), xb5_sets_v2:size(FilteredSet))
        end,
        AmountsToRemove
    ).
...
run_filtermap(Size, RefElements, Set) ->
    ...
    lists:foreach(
        fun({AmountToKeep, PercentageToMap}) ->
            ...
            FiltermappedSet = xb5_sets_v2:filtermap(FiltermapFun, Set),

            ExpectedElementsRemaining = lists:usort(lists:filtermap(FiltermapFun, RefElements)),
            ?assertListsCanonEqual(ExpectedElementsRemaining, xb5_sets_v2:to_list(FiltermappedSet)),

            ?assertEqual(length(ExpectedElementsRemaining), xb5_sets_v2:size(FiltermappedSet))
        end,
        ParamCombos
    ).
...
run_map(RefElements, Set) ->
    ...
    lists:foreach(
        fun(PercentageMapped) ->
            ...
            MappedSet = xb5_sets_v2:map(MapFun, Set),

            ExpectedMappedRef = lists:usort(lists:map(MapFun, RefElements)),

            ?assertEqual(
                length(ExpectedMappedRef),
                xb5_sets_v2:size(MappedSet)
            ),

            ?assertListsCanonEqual(
                ExpectedMappedRef,
                xb5_sets_v2:to_list(MappedSet)
            )
        end,
        PercentagesMapped
    ).
    ...
    StatsAcc =
        lists:foldl(
            fun(_, Acc) ->
                ...
                ?assertEqual(?STRUCTURE_TEST_BASE_SIZE, xb5_sets_v2:size(Set)),
