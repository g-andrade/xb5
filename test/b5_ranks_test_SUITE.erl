...
% -define(IMPROPER_LIST(V1, V2), [(V1) | b5_ranks_util:dialyzer_opaque_term(V2)]).
...
test_from_list_repeated_keys(_Config) ->
    Tree = b5_ranks:from_list([{1, a}, {2, b}, {1.0, c}, {4, d}]),
    %% Keys 1 and 1.0 are equal, so 1.0 should win (last one)
    ?assertEqual([{1.0, c}, {2, b}, {4, d}], b5_ranks:to_list(Tree)).

test_get_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertError({badkey, foobar}, b5_ranks:get(foobar, EmptyTree)),

    %% Test with multiple tree sizes
    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, NonExistentKeys) ->
            %% Test existent keys
            maps:fold(
                fun(Key, ExpectedValue, _) ->
                    Key2 = b5_ranks_test_helpers:randomly_switch_key_type(Key),
                    ?assertEqual(ExpectedValue, b5_ranks:get(Key2, Tree))
                end,
                ok,
                ExistentMap
            ),
            ...
            lists:foreach(
                fun(Key) ->
                    ?assertError({badkey, Key}, b5_ranks:get(Key, Tree))
                end,
                b5_ranks_test_helpers:take_random(NonExistentKeys, 10)
            )
        end
    ).
...
test_to_list_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual([], b5_ranks:to_list(EmptyTree)),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, _NonExistentKeys) ->
            Expected = b5_ranks_test_helpers:sort_by(
                maps:to_list(ExistentMap),
                fun({K, _}) -> K end
            ),
            ?assertEqual(Expected, b5_ranks:to_list(Tree))
        end
    ).

test_empty_function(_Config) ->
    %% Test that empty/0 is equivalent to new/0
    EmptyTree1 = b5_ranks:empty(),
    EmptyTree2 = b5_ranks:new(),
    ?assertEqual(EmptyTree2, EmptyTree1),
    ?assert(b5_ranks:is_empty(EmptyTree1)),
    ?assertEqual(0, b5_ranks:size(EmptyTree1)),
    ?assertEqual([], b5_ranks:to_list(EmptyTree1)).
...
test_constituent_parts(_Config) ->
    ...
    ?assertEqual(b5_ranks:size(Tree), b5_ranks:size(ReconstructedTree)),
    ...
    ?assertEqual(error, b5_ranks:to_constituent_parts(invalid_tree)),
    ?assertEqual(error, b5_ranks:to_constituent_parts(not_a_tree)),
    ?assertEqual(error, b5_ranks:to_constituent_parts({some, random, tuple})).
...
test_insert_operations(_Config) ->
    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, NonExistentKeys) ->
            ExistentKvs = b5_ranks_test_helpers:sort_by(
                maps:to_list(ExistentMap),
                fun({K, _}) -> K end
            ),
            ShuffledNonExistent = b5_ranks_test_helpers:shuffle(NonExistentKeys),
...
test_update_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertError({badkey, foobar}, b5_ranks:update(foobar, v, EmptyTree)),
...
test_update_with_3_operations(_Config) ->
    ...
    ?assertError(
        {badkey, foobar},
        b5_ranks:update_with(foobar, fun(V) -> V end, EmptyTree)
    ),
...
test_inserts(Tree, ExistentKvs, []) ->
    ?assertEqual(ExistentKvs, b5_ranks:to_list(Tree)),
    ?assertMatch({ok, _}, b5_ranks:validate(Tree)),
    ?assertEqual(length(ExistentKvs), b5_ranks:size(Tree));
test_inserts(Tree, ExistentKvs, [NonExistentKey | Rest]) ->
    %% Test that inserting existing keys fails
    ExistentSample = b5_ranks_test_helpers:take_random(ExistentKvs, 10),
    lists:foreach(
        fun({Key, _V}) ->
            Key2 = b5_ranks_test_helpers:randomly_switch_key_type(Key),
            ?assertError({key_exists, Key2}, b5_ranks:insert(Key2, v, Tree))
        end,
        ExistentSample
    ),

    %% Insert new key
    Value = b5_ranks_test_helpers:random_number(),
    InsertKey = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
    Tree2 = b5_ranks:insert(InsertKey, Value, Tree),

    %% Verify insertion
    NewExistentKvs = orddict:store(InsertKey, Value, ExistentKvs),
    ?assertEqual(NewExistentKvs, b5_ranks:to_list(Tree2)),
    ?assertEqual(length(NewExistentKvs), b5_ranks:size(Tree2)),
    ?assertNot(b5_ranks:is_empty(Tree2)),
...
test_inserts_with(Tree, ExistentKvs, [NonExistentKey | Rest]) ->
    ...
    lists:foreach(
        fun({Key, _V}) ->
            Key2 = b5_ranks_test_helpers:randomly_switch_key_type(Key),
            ValueFun = fun b5_ranks_test_helpers:error_not_to_be_called/0,
            ?assertError(
                {key_exists, Key2},
                b5_ranks:insert_with(Key2, ValueFun, Tree)
            )
        end,
        ExistentSample
    ),
    ...
    InsertKey = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
    Tree2 = b5_ranks:insert_with(InsertKey, ValueFun, Tree),
...
test_updates(Tree, [{Key, _V} | NextKeys], ExistentKvs, NonExistentKeys) ->
    NewValue = b5_ranks_test_helpers:random_number(),
    SizeBefore = b5_ranks:size(Tree),

    UpdateKey = b5_ranks_test_helpers:randomly_switch_key_type(Key),
    Tree2 = b5_ranks:update(UpdateKey, NewValue, Tree),
    ?assertEqual(SizeBefore, b5_ranks:size(Tree2)),
    ?assertNot(b5_ranks:is_empty(Tree2)),
    ...
    NonExistentSample = b5_ranks_test_helpers:take_random(NonExistentKeys, 10),
    lists:foreach(
        fun(NonExistentKey) ->
            Key2 = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
            ?assertError({badkey, Key2}, b5_ranks:update(Key2, NewValue, Tree2))
        end,
        NonExistentSample
    ),
...
test_updates_with3(Tree, [{Key, PrevV} | NextKeys], ExistentKvs, NonExistentKeys) ->
    ...
    Tree2 = b5_ranks:update_with(UpdateKey, UpdateFun, Tree),
    ?assertEqual(SizeBefore, b5_ranks:size(Tree2)),
    ?assertNot(b5_ranks:is_empty(Tree2)),
    ...
    lists:foreach(
        fun(NonExistentKey) ->
            ...
            ?assertError(
                {badkey, Key2},
                b5_ranks:update_with(Key2, fun(V) -> V end, Tree2)
            )
        end,
        NonExistentSample
    ),
    ...
    case {ExistentAttempts, NonExistentKeys} of
        {[{ExistentKey, ExistentPrevV} | NextExistent], [NonExistentKey | NextNonExistent]} ->
            case rand:uniform(2) of
                ...
                1 ->
                    UpdateKey = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
                    UpdateFun = fun b5_ranks_test_helpers:error_not_to_be_called/1,
                    Tree2 = b5_ranks:update_with(UpdateKey, UpdateFun, NewValue, Tree),
                    ?assertEqual(SizeBefore + 1, b5_ranks:size(Tree2)),
                    ?assertNot(b5_ranks:is_empty(Tree2)),
                ...
                2 ->
                    UpdateKey = b5_ranks_test_helpers:randomly_switch_key_type(ExistentKey),
                    UpdateFun = fun(TreeV) ->
                        ?assertEqual(ExistentPrevV, TreeV),
                        NewValue
                    end,
                    Tree2 = b5_ranks:update_with(UpdateKey, UpdateFun, unused_default, Tree),
                    ?assertEqual(SizeBefore, b5_ranks:size(Tree2)),
                    ?assertNot(b5_ranks:is_empty(Tree2)),
...
test_smallest_largest_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertError(empty_tree, b5_ranks:smallest(EmptyTree)),
    ?assertError(empty_tree, b5_ranks:largest(EmptyTree)),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, _NonExistentKeys) ->
            {SmallestK, SmallestV} = hd(
                b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) -> K end)
            ),
            {LargestK, LargestV} = lists:last(
                b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) -> K end)
            ),
            ?assertEqual({SmallestK, SmallestV}, b5_ranks:smallest(Tree)),
            ?assertEqual({LargestK, LargestV}, b5_ranks:largest(Tree))
        end
    ).

test_delete_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertError({badkey, foobar}, b5_ranks:delete(foobar, EmptyTree)),
...
test_keys_values_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual([], b5_ranks:keys(EmptyTree)),
    ?assertEqual([], b5_ranks:values(EmptyTree)),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, NonExistentKeys) ->
            ExpectedKeys = b5_ranks_test_helpers:sort_by(maps:keys(ExistentMap), fun(K) -> K end),
            test_keys_impl(Tree, ExpectedKeys, b5_ranks_test_helpers:shuffle(NonExistentKeys)),

            ExpectedAux = gb_trees:from_orddict(
                b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) -> K end)
            ),
            test_values_impl(Tree, ExpectedAux, b5_ranks_test_helpers:shuffle(NonExistentKeys))
        end
    ).

test_smaller_larger_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual(none, b5_ranks:smaller(any_key, EmptyTree)),
    ?assertEqual(none, b5_ranks:larger(any_key, EmptyTree)),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, NonExistentKeys) ->
            AllKeys = b5_ranks_test_helpers:shuffle(maps:keys(ExistentMap) ++ NonExistentKeys),
...
test_take_smallest_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertError(empty_tree, b5_ranks:take_smallest(EmptyTree)),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, _NonExistentKeys) ->
            ExistentKvs = b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) ->
                K
            end),
...
test_take_largest_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertError(empty_tree, b5_ranks:take_largest(EmptyTree)),
...
test_take_operations(_Config) ->
    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, NonExistentKeys) ->
            ExistentKvs = b5_ranks_test_helpers:shuffle(maps:to_list(ExistentMap)),
...
test_iterator_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual(none, b5_ranks:next(b5_ranks:iterator(EmptyTree))),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, _NonExistentKeys) ->
            Iterator = b5_ranks:iterator(Tree),
            ExpectedKvs = b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) ->
                K
            end),
            Next = b5_ranks:next(Iterator),
...
test_iterator_ordered_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual(none, b5_ranks:next(b5_ranks:iterator(EmptyTree, ordered))),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, _NonExistentKeys) ->
            Iterator = b5_ranks:iterator(Tree, ordered),
            ExpectedKvs = b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) ->
                K
            end),
            Next = b5_ranks:next(Iterator),
...
test_iterator_reversed_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual(none, b5_ranks:next(b5_ranks:iterator(EmptyTree, reversed))),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, _NonExistentKeys) ->
            Iterator = b5_ranks:iterator(Tree, reversed),
...
test_iterator_from_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual(none, b5_ranks:next(b5_ranks:iterator_from(any_key, EmptyTree))),
    ...
    b5_ranks_test_helpers:for_each_tree(
        ...
        fun(Tree, ExistentMap, NonExistentKeys) ->
            ...
            lists:foreach(
                fun(StartingKey) ->
                    StartingKey2 = b5_ranks_test_helpers:randomly_switch_key_type(StartingKey),
                    Iterator = b5_ranks:iterator_from(StartingKey2, Tree),

                    ExpectedKvs = lists:filter(
                        fun({K, _}) -> K >= StartingKey2 end,
                        b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) ->
                            K
                        end)
                    ),
...
test_iterator_from_ordered_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual(none, b5_ranks:next(b5_ranks:iterator_from(any_key, EmptyTree, ordered))),
    ...
    b5_ranks_test_helpers:for_each_tree(
        ...
        fun(Tree, ExistentMap, NonExistentKeys) ->
            ...
            lists:foreach(
                fun(StartingKey) ->
                    StartingKey2 = b5_ranks_test_helpers:randomly_switch_key_type(StartingKey),
                    Iterator = b5_ranks:iterator_from(StartingKey2, Tree, ordered),
...
test_iterator_from_reversed_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    ?assertEqual(none, b5_ranks:next(b5_ranks:iterator_from(any_key, EmptyTree, reversed))),
    ...
    b5_ranks_test_helpers:for_each_tree(
        ...
        fun(Tree, ExistentMap, NonExistentKeys) ->
            ...
            lists:foreach(
                fun(StartingKey) ->
                    StartingKey2 = b5_ranks_test_helpers:randomly_switch_key_type(StartingKey),
                    Iterator = b5_ranks:iterator_from(StartingKey2, Tree, reversed),
...
test_foldl_foldr_operations(_Config) ->
    ...
    EmptyTree = b5_ranks:new(),
    ?assertEqual([], b5_ranks:foldl(FoldFun, [], EmptyTree)),
    ?assertEqual([], b5_ranks:foldr(FoldFun, [], EmptyTree)),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, _NonExistentKeys) ->
            AccFoldl = b5_ranks:foldl(FoldFun, [], Tree),
            ExpectedFoldl = lists:reverse(
                b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) -> K end)
            ),
            ?assertEqual(ExpectedFoldl, AccFoldl),

            AccFoldr = b5_ranks:foldr(FoldFun, [], Tree),
            ExpectedFoldr = b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) ->
                K
            end),
...
test_map_operations(_Config) ->
    EmptyTree = b5_ranks:new(),
    MappedEmptyTree = b5_ranks:map(fun b5_ranks_test_helpers:error_not_to_be_called/2, EmptyTree),
    ?assertEqual(0, b5_ranks:size(MappedEmptyTree)),
    ?assert(b5_ranks:is_empty(MappedEmptyTree)),
    ?assertEqual([], b5_ranks:to_list(MappedEmptyTree)),

    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, _NonExistentKeys) ->
            NewValues = maps:from_list([
                {K, b5_ranks_test_helpers:random_number()}
             || {K, _V} <- maps:to_list(ExistentMap)
            ]),
            MapFun = fun(K, _V) -> maps:get(K, NewValues) end,
            MappedTree = b5_ranks:map(MapFun, Tree),

            ?assertEqual(
                b5_ranks_test_helpers:sort_by(maps:to_list(NewValues), fun({K, _}) -> K end),
                b5_ranks:to_list(MappedTree)
            ),
            ?assertEqual(maps:size(ExistentMap), b5_ranks:size(MappedTree)),
            ?assertNot(b5_ranks:is_empty(MappedTree))
        end
    ).
...
test_enter_operations(_Config) ->
    b5_ranks_test_helpers:for_each_tree(
        ?REGULAR_TREE_SIZES,
        fun(Tree, ExistentMap, NonExistentKeys) ->
            KeysToEnter = b5_ranks_test_helpers:shuffle(maps:keys(ExistentMap) ++ NonExistentKeys),
            ExistentKvs = b5_ranks_test_helpers:sort_by(maps:to_list(ExistentMap), fun({K, _}) ->
                K
            end),
        ...
        fun(Tree, ExistentMap, NonExistentKeys) ->
            maps:fold(
                fun(Key, _ExpectedValue, _) ->
                    ?assert(b5_ranks:is_defined(Key, Tree))
                end,
                ok,
                ExistentMap
            ),
            ...
            lists:foreach(
                fun(Key) ->
                    ?assertNot(b5_ranks:is_defined(Key, Tree))
                end,
                NonExistentKeys
            )
        end
...
test_lookup_operations(_Config) ->
    b5_ranks_test_helpers:for_each_tree(
        ...
        fun(Tree, ExistentMap, NonExistentKeys) ->
            maps:fold(
                fun(Key, ExpectedValue, _) ->
                    ?assertEqual({value, ExpectedValue}, b5_ranks:lookup(Key, Tree))
                end,
                ok,
                ExistentMap
            ),
            ...
            lists:foreach(
                fun(Key) ->
                    ?assertEqual(none, b5_ranks:lookup(Key, Tree))
                end,
                NonExistentKeys
            )
        end
...
test_delete_each(Tree, ExistentMap, NonExistentKeys) ->
    maps:fold(
        fun(Key, ExpectedValue, _) ->
            DeleteKey = b5_ranks_test_helpers:randomly_switch_key_type(Key),
            ?assertEqual(ExpectedValue, b5_ranks:get(DeleteKey, Tree)),

            ExpectedListAfter = b5_ranks_test_helpers:sort_by(
            ...
            NonExistentSample = b5_ranks_test_helpers:take_random(NonExistentKeys, 20),
            lists:foreach(
                fun(NonExistentKey) ->
                    NonExistentKey2 = b5_ranks_test_helpers:randomly_switch_key_type(
                        NonExistentKey
                    ),
                    ?assertError({badkey, NonExistentKey2}, b5_ranks:delete(NonExistentKey2, Tree2))
                end,
                NonExistentSample
            )
        end,
...
test_delete_all(Tree, ExistentMap, NonExistentKeys) ->
    ...
    ElementsToDelete = b5_ranks_test_helpers:shuffle(KvList),
    ElementsToExpect = b5_ranks_test_helpers:sort_by(KvList, fun({K, _}) -> K end),
...
test_delete_all_impl(Tree, [{Key, _ExpectedV} | Next], ElementsToExpected, NonExistentKeys) ->
    ...
    ?assertEqual(ElementsToExpected2, b5_ranks:to_list(Tree2)),
    ?assertMatch({ok, _}, b5_ranks:validate(Tree2)),
    ?assertEqual(length(ElementsToExpected2), b5_ranks:size(Tree2)),
    ?assertEqual(ElementsToExpected2 =:= [], b5_ranks:is_empty(Tree2)),
    ...
    lists:foreach(
        fun(NonExistentKey) ->
            NonExistentKey2 = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
            ?assertError({badkey, NonExistentKey2}, b5_ranks:delete(NonExistentKey2, Tree2))
        end,
        NonExistentSample
    ),
...
test_delete_all_impl(Tree, [], [], NonExistentKeys) ->
    ?assertEqual([], b5_ranks:to_list(Tree)),
    ?assertMatch({ok, _}, b5_ranks:validate(Tree)),
    ?assertEqual(0, b5_ranks:size(Tree)),
    ?assert(b5_ranks:is_empty(Tree)),

    NonExistentSample = b5_ranks_test_helpers:take_random(NonExistentKeys, 10),
    lists:foreach(
        fun(NonExistentKey) ->
            NonExistentKey2 = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
            ?assertError({badkey, NonExistentKey2}, b5_ranks:delete(NonExistentKey2, Tree))
        end,
        NonExistentSample
    ).

test_keys_impl(Tree, ExpectedKeys, [NonExistentKey | Next]) ->
    ?assertEqual(ExpectedKeys, b5_ranks:keys(Tree)),

    InsertKey = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
    NewValue = b5_ranks_test_helpers:random_number(),

    Tree2 = b5_ranks:insert(InsertKey, NewValue, Tree),
...
test_values_impl(Tree, ExpectedAux, [NonExistentKey | Next]) ->
    ?assertEqual(gb_trees:values(ExpectedAux), b5_ranks:values(Tree)),
...
test_smaller_impl(Tree, ExistentMap, [Key | Next]) ->
    ...
    LookupKey = b5_ranks_test_helpers:randomly_switch_key_type(Key),
    ?assertEqual(ExpectedPair, b5_ranks:smaller(LookupKey, Tree)),
...
test_larger_impl(Tree, ExistentMap, [Key | Next]) ->
    ...
    LookupKey = b5_ranks_test_helpers:randomly_switch_key_type(Key),
    ?assertEqual(ExpectedPair, b5_ranks:larger(LookupKey, Tree)),
...
test_take_all_smallest(Tree, [{ExpectedKey, ExpectedValue} | Next] = ExistentKvs) ->
    ?assertEqual(length(ExistentKvs), b5_ranks:size(Tree)),
    ?assertNot(b5_ranks:is_empty(Tree)),

    {TakenKey, TakenValue, Tree2} = b5_ranks:take_smallest(Tree),
...
test_take_all_smallest(Tree, []) ->
    ?assertEqual(0, b5_ranks:size(Tree)),
    ?assert(b5_ranks:is_empty(Tree)),
    ?assertError(empty_tree, b5_ranks:take_smallest(Tree)).

test_take_all_largest(Tree, [{ExpectedKey, ExpectedValue} | Next] = ExistentKvs) ->
    ?assertEqual(length(ExistentKvs), b5_ranks:size(Tree)),
    ?assertNot(b5_ranks:is_empty(Tree)),

    {TakenKey, TakenValue, Tree2} = b5_ranks:take_largest(Tree),
    ?assertEqual(ExpectedKey, TakenKey),
    ?assertEqual(ExpectedValue, TakenValue),
...
test_take_all_largest(Tree, []) ->
    ?assertEqual(0, b5_ranks:size(Tree)),
    ?assert(b5_ranks:is_empty(Tree)),
    ?assertError(empty_tree, b5_ranks:take_largest(Tree)).

test_take_all(Tree, ExistentKvs, NonExistentKeys) ->
    ?assertEqual(length(ExistentKvs), b5_ranks:size(Tree)),
    ?assertEqual(ExistentKvs =:= [], b5_ranks:is_empty(Tree)),

    NonExistentSample = b5_ranks_test_helpers:take_random(NonExistentKeys, 20),
    lists:foreach(
        fun(NonExistentKey) ->
            NonExistentKey2 = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
            ?assertError({badkey, NonExistentKey2}, b5_ranks:take(NonExistentKey2, Tree))
        end,
        NonExistentSample
    ),
    ...
    case ExistentKvs of
        [{Key, Value} | Next] ->
            LookupKey = b5_ranks_test_helpers:randomly_switch_key_type(Key),
            {TakenValue, Tree2} = b5_ranks:take(LookupKey, Tree),
...
test_take_any_impl(Tree, ExistentKvs, NonExistentKeys) ->
    ...
    lists:foreach(
        fun(NonExistentKey) ->
            NonExistentKey2 = b5_ranks_test_helpers:randomly_switch_key_type(NonExistentKey),
            ?assertEqual(error, b5_ranks:take_any(NonExistentKey2, Tree))
        end,
        NonExistentSample
    ),
    ...
    case ExistentKvs of
        [{Key, Value} | Next] ->
            LookupKey = b5_ranks_test_helpers:randomly_switch_key_type(Key),
            {TakenValue, Tree2} = b5_ranks:take_any(LookupKey, Tree),
            ?assertEqual(Value, TakenValue),
...
test_iterator_impl([{ExpectedK, ExpectedV} | Next], {NextK, NextV, Iter}) ->
    ...
    test_iterator_impl(Next, b5_ranks:next(Iter));
...
test_enter_impl(Tree, [Key | NextKeys], ExistentKvs, NonExistentKeys) ->
    ...
    EnterKey = b5_ranks_test_helpers:randomly_switch_key_type(Key),
    EnterValue = b5_ranks_test_helpers:random_number(),
    Tree2 = b5_ranks:enter(EnterKey, EnterValue, Tree),
