-module(xb5_trees_v2_test_SUITE).
...
test_construction(_Config) ->
    foreach_tested_size(
        fun(Size, RefKvs) ->
            Tree = xb5_trees_v2:from_list(RefKvs),
            ?assertKvListsCanonEqual(RefKvs, xb5_trees_v2:to_list(Tree)),
            ?assertEqual(Size, xb5_trees_v2:size(Tree)),
            ?assertEqual(Size =:= 0, xb5_trees_v2:is_empty(Tree)),
            ?assertEqual(Tree, new_tree_from_each_inserted(RefKvs)),

            ?assertKvListsCanonEqual(
                RefKvs, xb5_trees_v2:to_list(xb5_trees_v2:from_orddict(RefKvs))
            ),

            _ = (Size =:= 0 andalso ?assertEqual(Tree, xb5_trees_v2:empty()))
        end
    ).
    ...
    foreach_test_tree(
        fun(Size, RefKvs, Tree) ->
            foreach_existing_pair(
                fun(Key, Value) ->
                    ?assertEqual(true, xb5_trees_v2:is_defined(Key, Tree)),
                    ?assertEqual(Value, xb5_trees_v2:get(Key, Tree)),
                    ?assertEqual({value, Value}, xb5_trees_v2:lookup(Key, Tree))
                end,
                RefKvs,
                Size
            ),
            ...
            foreach_non_existent_key(
                fun(Key) ->
                    ?assertEqual(false, xb5_trees_v2:is_defined(Key, Tree)),
                    ?assertError({badkey, Key}, xb5_trees_v2:get(Key, Tree)),
                    ?assertEqual(none, xb5_trees_v2:lookup(Key, Tree))
                end,
                RefKvs,
                100
            )
...
test_insert(_Config) ->
    foreach_test_tree(
        fun(Size, RefKvs, Tree) ->
            foreach_existing_pair(
                fun(Key, _Value) ->
                    ?assertError({key_exists, Key}, xb5_trees_v2:insert(Key, new_value, Tree))
                end,
                RefKvs,
                min(50, Size)
            ),
            ...
            foreach_non_existent_key(
                fun(Key) ->
                    ...
                    Tree2 = xb5_trees_v2:insert(Key, Value, Tree),
                    ?assertEqual(Size + 1, xb5_trees_v2:size(Tree2)),
                    ?assertKvListsCanonEqual(
                        add_to_sorted_list(Key, Value, RefKvs),
                        xb5_trees_v2:to_list(Tree2)
                    )
                end,
                RefKvs,
                50
        ...
        fun(Size, RefKvs, Tree) ->
            foreach_existing_pair(
                fun(Key, _Value) ->
                    ...
                    ?assertError({key_exists, Key}, xb5_trees_v2:insert_with(Key, Fun, Tree))
                end,
                RefKvs,
                min(50, Size)
            ),
            ...
            foreach_non_existent_key(
                fun(Key) ->
                    Value = {new_value, make_ref()},
                    Fun = fun() -> Value end,
                    Tree2 = xb5_trees_v2:insert_with(Key, Fun, Tree),
                    ?assertEqual(Size + 1, xb5_trees_v2:size(Tree2)),
                    ?assertKvListsCanonEqual(
                        add_to_sorted_list(Key, Value, RefKvs),
                        xb5_trees_v2:to_list(Tree2)
...
test_delete_sequential(_Config) ->
    foreach_test_tree(
        fun(_Size, RefKvs, Tree) ->
            ...
            {TreeN, []} =
                lists:foldl(
                    fun(Key, {Tree1, RemainingKvs1}) ->
                        ...
                        Tree2 = xb5_trees_v2:delete(Key, Tree1),
                        RemainingKvs2 = remove_from_sorted_list(Key, RemainingKvs1),
                        ?assertKvListsCanonEqual(RemainingKvs2, xb5_trees_v2:to_list(Tree2)),
                        ?assertEqual(length(RemainingKvs2), xb5_trees_v2:size(Tree2)),
                        ?assertEqual(RemainingKvs2 =:= [], xb5_trees_v2:is_empty(Tree2)),

                        ?assertEqual(Tree2, xb5_trees_v2:delete_any(Key, Tree1)),
            ...
            ?assertEqual([], xb5_trees_v2:to_list(TreeN)),
            ?assertEqual(0, xb5_trees_v2:size(TreeN)),
            ?assertEqual(true, xb5_trees_v2:is_empty(TreeN)),
        ...
        fun(Size, RefKvs, Tree) ->
            foreach_existing_pair(
                fun(Key, _Value) ->
                    ...
                    Tree2 = xb5_trees_v2:update(Key, NewValue, Tree),

                    ?assertEqual(Size, xb5_trees_v2:size(Tree2)),
            ...
            foreach_non_existent_key(
                fun(Key) ->
                    NewValue = {new_value, make_ref()},
                    ?assertError({badkey, Key}, xb5_trees_v2:update(Key, NewValue, Tree))
                end,
                RefKvs,
                50
            )
        ...
        fun(Size, RefKvs, Tree) ->
            foreach_existing_pair(
                fun(Key, Value) ->
                    ...
                    Tree2 = xb5_trees_v2:update_with(Key, Fun, Tree),
            ...
            foreach_non_existent_key(
                fun(Key) ->
                    ...
                    ?assertError({badkey, Key}, xb5_trees_v2:update_with(Key, Fun, Tree))
                end,
                RefKvs,
                50
            )
        ...
        fun(Size, RefKvs, Tree) ->
            foreach_existing_pair(
                fun(Key, Value) ->
                    ...
                    Tree2 = xb5_trees_v2:update_with(Key, Fun, InitValue, Tree),
    ...
    foreach_test_tree(
        fun(Size, RefKvs, Tree) ->
            foreach_existing_pair(
                fun(Key, _Value) ->
                    NewValue = {new_value, make_ref()},
                    Tree2 = xb5_trees_v2:enter(Key, NewValue, Tree),
            ...
            foreach_non_existent_key(
                fun(Key) ->
                    Value = {new_value, make_ref()},
                    Tree2 = xb5_trees_v2:enter(Key, Value, Tree),
                    ?assertEqual(Size + 1, xb5_trees_v2:size(Tree2)),
                    ?assertKvListsCanonEqual(
                        add_to_sorted_list(Key, Value, RefKvs),
                        xb5_trees_v2:to_list(Tree2)
    ...
    foreach_test_tree(
        fun(_Size, RefKvs, Tree) ->
            ?assertEqual(
                ...
                lists:map(fun canon_key/1, xb5_trees_v2:keys(Tree))
            )
        end
    ).
            ...
            {TreeN, []} =
                lists:foldl(
                    fun({Key, Value}, {Tree1, RemainingKvs1}) ->
                        ...
                        {TakenValue, Tree2} = xb5_trees_v2:take(Key, Tree1),
                        ...
                        ?assertEqual({TakenValue, Tree2}, xb5_trees_v2:take_any(Key, Tree1)),
...
test_smallest(_Config) ->
    foreach_test_tree(
        fun
            (0, _RefKvs, Tree) ->
                ?assertError(empty_tree, xb5_trees_v2:smallest(Tree));
            %
            (_Size, RefKvs, Tree) ->
                ?assertCanonEqual(hd(RefKvs), xb5_trees_v2:smallest(Tree))
        end
    ).
...
test_largest(_Config) ->
    foreach_test_tree(
        fun
            (0, _RefKvs, Tree) ->
                ?assertError(empty_tree, xb5_trees_v2:largest(Tree));
            %
            (_Size, RefKvs, Tree) ->
                ?assertCanonEqual(lists:last(RefKvs), xb5_trees_v2:largest(Tree))
        end
    ).
...
test_iterator_reversed(_Config) ->
    foreach_test_tree(
        fun(_Size, RefKvs, Tree) ->
            Iter = xb5_trees_v2:iterator(Tree, reversed),
...
test_balance(_Config) ->
    foreach_test_tree(
        fun(_Size, _RefKvs, Tree) ->
            ...
            ?assertEqual(Tree, xb5_trees_v2:balance(Tree))
        end
    ).
...
test_rewrap(_Config) ->
    ?assertMatch({error, _}, xb5_trees_v2:unwrap(xb5_sets:new())),
    ?assertMatch({error, _}, xb5_trees_v2:unwrap(xb5_bag:new())),
    ?assertMatch({error, _}, xb5_trees_v2:unwrap({xb5_tree, -1, xb5_trees_v2_node:new()})),
    ?assertMatch({error, _}, xb5_trees_v2:unwrap({xb5_tree, 2, xb5_trees_v2_node:new()})),
    ?assertMatch({error, _}, xb5_trees_v2:unwrap({xb5_tree, 2, make_ref()})),

    foreach_test_tree(
        fun(_Size, _RefElements, Col) ->
            {ok, Unwrapped} = xb5_trees_v2:unwrap(Col),
            ?assertEqual(Col, xb5_trees_v2:wrap(Unwrapped))
        end
    ).
        ...
        run_structure_test(
            fun(RefKvs) ->
                ...
                case rand:uniform(4) of
                    ...
                    2 ->
                        xb5_trees_v2:from_list(RefKvs);
                    3 ->
                        new_tree_from_each_inserted(lists:reverse(RefKvs));
                    4 ->
                        xb5_trees_v2:from_list(lists:reverse(RefKvs))
                end
            end
        ),
...
test_structure_built_from_orddict(_Config) ->
    CondensedStats =
        run_structure_test(
            fun(RefKvs) ->
                ?assertEqual(RefKvs, lists:keysort(1, RefKvs)),
                xb5_trees_v2:from_orddict(RefKvs)
            end
        ),
    ...
    CondensedStats =
        run_structure_test(
            fun(RefKvs) ->
                ...
                AmountToDelete = xb5_trees_v2:size(Tree1) div 2,
                KeysToDelete = list_keys(lists:sublist(RefKvs, AmountToDelete)),
                lists:foldl(fun xb5_trees_v2:delete/2, Tree1, KeysToDelete)
            end,
            [{size_multiplier, 2}]
        ),
...
foreach_test_tree(Fun, Opts) ->
    foreach_tested_size(
        fun(Size, RefKvs) ->
            Tree = xb5_trees_v2:from_list(maybe_shuffle_list_for_new_tree(RefKvs)),
            ?assertEqual(Size, xb5_trees_v2:size(Tree)),

            Stats = xb5_trees_v2:structural_stats(Tree),
...
new_tree_from_each_inserted(List) ->
    Tree = xb5_trees_v2:new(),

    ?assertEqual(0, xb5_trees_v2:size(Tree)),
    ?assertEqual(true, xb5_trees_v2:is_empty(Tree)),

    new_tree_from_each_inserted_recur(List, Tree).

new_tree_from_each_inserted_recur([{Key, Value} | Next], Tree) ->
    UpdatedTree = xb5_trees_v2:insert(Key, Value, Tree),
...
run_construction_repeated_test(Size, [KeyToRepeat | Next], RefKvs) ->
    ...
    Tree = xb5_trees_v2:from_list(List),

    ?assertEqual(Size, xb5_trees_v2:size(Tree)),

    ?assertKvListsCanonEqual(
        sort_kv_list_keep_last_repeated(List),
        xb5_trees_v2:to_list(Tree)
    ),

    %%%

    ShuffledList = list_shuffle(List),
    TreeShuffled = xb5_trees_v2:from_list(ShuffledList),

    ?assertEqual(Size, xb5_trees_v2:size(TreeShuffled)),

    ?assertKvListsCanonEqual(
        sort_kv_list_keep_last_repeated(ShuffledList),
        xb5_trees_v2:to_list(TreeShuffled)
    ),
...
test_extensive_construction_from_orddict_recur(FirstKey, Acc) when FirstKey > 0 ->
    ...
    Tree = xb5_trees_v2:from_orddict(List),

    ?assertKvListsCanonEqual(List, xb5_trees_v2:to_list(Tree)),

    _ = xb5_trees_v2:structural_stats(Tree),
...
test_delete_non_existing_keys(Tree, RemainingKeys, Amount) when Amount > 0 ->
    ...
    case lists:any(fun({K, _}) -> K == Key end, RemainingKeys) of
        false ->
            ?assertError({badkey, Key}, xb5_trees_v2:delete(Key, Tree)),
            ?assertEqual(Tree, xb5_trees_v2:delete_any(Key, Tree)),
...
test_take_non_existing_keys(Tree, RemainingKeys, Amount) when Amount > 0 ->
    ...
    case lists:any(fun({K, _}) -> K == Key end, RemainingKeys) of
        false ->
            ?assertError({badkey, Key}, xb5_trees_v2:take(Key, Tree)),
            ?assertEqual(error, xb5_trees_v2:take_any(Key, Tree)),
...
run_smaller(RefKvs, Tree) ->
    case RefKvs of
        [] ->
            Key = new_key(),
            ?assertEqual(none, xb5_trees_v2:smaller(Key, Tree));
        %
        [{SingleKey, _} = SinglePair] ->
            ?assertEqual(none, xb5_trees_v2:smaller(randomly_switch_number_type(SingleKey), Tree)),

            LargerKey = key_larger(SingleKey),
            ?assertEqual(SinglePair, xb5_trees_v2:smaller(LargerKey, Tree)),

            SmallerKey = key_smaller(SingleKey),
            ?assertEqual(none, xb5_trees_v2:smaller(SmallerKey, Tree));
        %
        [{FirstKey, FirstValue} | Next] ->
            ?assertEqual(none, xb5_trees_v2:smaller(randomly_switch_number_type(FirstKey), Tree)),
...
run_smaller_recur(ExpectedKey, ExpectedValue, [{LastKey, LastValue}], Tree) ->
    ?assertCanonEqual(
        ...
        xb5_trees_v2:smaller(randomly_switch_number_type(LastKey), Tree)
    ),

    LargerKey = key_larger(LastKey),
    ?assert(LargerKey > LastKey),
    ?assert(xb5_trees_v2:smaller(LargerKey, Tree) == {LastKey, LastValue});
run_smaller_recur(ExpectedKey, ExpectedValue, [{Key, Value} | Next], Tree) ->
    ?assert(
        xb5_trees_v2:smaller(randomly_switch_number_type(Key), Tree) == {ExpectedKey, ExpectedValue}
    ),
    ...
    case key_in_between(ExpectedKey, Key) of
        {found, InBetween} ->
            ...
            ?assertCanonEqual({ExpectedKey, ExpectedValue}, xb5_trees_v2:smaller(InBetween, Tree));
        %
        none ->
            ok
    end,
...
run_larger(RefKvs, Tree) ->
    case lists:reverse(RefKvs) of
        [] ->
            Key = new_key(),
            ?assertEqual(none, xb5_trees_v2:larger(Key, Tree));
        %
        [{SingleKey, SingleValue}] ->
            ?assertEqual(none, xb5_trees_v2:larger(SingleKey, Tree)),

            LargerKey = key_larger(SingleKey),
            ?assertEqual(none, xb5_trees_v2:larger(LargerKey, Tree)),

            SmallerKey = key_smaller(SingleKey),
            ?assertEqual({SingleKey, SingleValue}, xb5_trees_v2:larger(SmallerKey, Tree));
        %
        [{LastKey, LastValue} | Next] ->
            ?assertEqual(none, xb5_trees_v2:larger(randomly_switch_number_type(LastKey), Tree)),
...
run_larger_recur(ExpectedKey, ExpectedValue, [{FirstKey, FirstValue}], Tree) ->
    ?assertCanonEqual(
        {ExpectedKey, ExpectedValue},
        xb5_trees_v2:larger(randomly_switch_number_type(FirstKey), Tree)
    ),

    SmallerKey = key_smaller(FirstKey),
    ?assert(SmallerKey < FirstKey),
    ?assertEqual({FirstKey, FirstValue}, xb5_trees_v2:larger(SmallerKey, Tree));
run_larger_recur(ExpectedKey, ExpectedValue, [{Key, Value} | Next], Tree) ->
    ?assertEqual(
        {ExpectedKey, ExpectedValue}, xb5_trees_v2:larger(randomly_switch_number_type(Key), Tree)
    ),
    ...
    case key_in_between(Key, ExpectedKey) of
        {found, InBetween} ->
            ?assert(InBetween < ExpectedKey),
            ?assert(InBetween > Key),
            ?assertCanonEqual({ExpectedKey, ExpectedValue}, xb5_trees_v2:larger(InBetween, Tree));
        %
        none ->
            ok
    end,
...
run_take_smallest([{ExpectedKey, ExpectedValue} | Next], Tree) ->
    {TakenKey, TakenValue, Tree2} = xb5_trees_v2:take_smallest(Tree),
    ?assertEqual({ExpectedKey, ExpectedValue}, {TakenKey, TakenValue}),
    ?assertEqual(length(Next), xb5_trees_v2:size(Tree2)),
    run_take_smallest(Next, Tree2);
run_take_smallest([], Tree) ->
    ?assertError(empty_tree, xb5_trees_v2:take_smallest(Tree)).

run_take_largest([{ExpectedKey, ExpectedValue} | Next], Tree) ->
    {TakenKey, TakenValue, Tree2} = xb5_trees_v2:take_largest(Tree),
    ?assertEqual({ExpectedKey, ExpectedValue}, {TakenKey, TakenValue}),
    ?assertEqual(length(Next), xb5_trees_v2:size(Tree2)),
    run_take_largest(Next, Tree2);
run_take_largest([], Tree) ->
    ?assertError(empty_tree, xb5_trees_v2:take_largest(Tree)).
...
new_iterator_from(Key, Tree) ->
    Iter = xb5_trees_v2:iterator_from(Key, Tree),
    ?assertEqual(Iter, xb5_trees_v2:iterator_from(Key, Tree, ordered)),
    Iter.
...
run_iterator_from_reversed(RefKvs, Tree) ->
    case lists:reverse(RefKvs) of
        [] ->
            Iter = xb5_trees_v2:iterator_from(new_key(), Tree, reversed),
            ?assertEqual([], iterate(Iter));
        %
        [{SingleKey, _}] ->
            Iter = xb5_trees_v2:iterator_from(
                randomly_switch_number_type(SingleKey), Tree, reversed
            ),
            ?assertKvListsCanonEqual(RefKvs, iterate(Iter)),

            SmallerKey = key_smaller(SingleKey),
            Iter2 = xb5_trees_v2:iterator_from(SmallerKey, Tree, reversed),
            ?assertEqual([], iterate(Iter2)),

            LargerKey = key_larger(SingleKey),
            Iter3 = xb5_trees_v2:iterator_from(LargerKey, Tree, reversed),
            ?assertKvListsCanonEqual(RefKvs, iterate(Iter3));
        %
        [{LastKey, _} | _] = ReverseRefKvs ->
            LargerKey = key_larger(LastKey),
            Iter = xb5_trees_v2:iterator_from(LargerKey, Tree, reversed),
...
run_iterator_from_reversed_recur([{Key2, _} | [{Key1, _} | _] = Tail] = List, Tree) ->
    Iter = xb5_trees_v2:iterator_from(Key2, Tree, reversed),
    ?assertKvListsCanonEqual(List, iterate(Iter)),
    ...
    case key_in_between(Key1, Key2) of
        {found, InBetween} ->
            ?assert(InBetween > Key1),
            ?assert(InBetween < Key2),
            Iter2 = xb5_trees_v2:iterator_from(InBetween, Tree, reversed),
...
run_iterator_from_reversed_first_key(FirstKey, FirstValue, Tree) ->
    Iter = xb5_trees_v2:iterator_from(randomly_switch_number_type(FirstKey), Tree, reversed),
...
new_iterator(Tree) ->
    Iter = xb5_trees_v2:iterator(Tree),
    ?assertEqual(Iter, xb5_trees_v2:iterator(Tree, ordered)),
    Iter.

iterate(Iter) ->
    case xb5_trees_v2:next(Iter) of
        {Key, Value, Iter2} ->
    ...
    StatsAcc =
        lists:foldl(
            fun(_, Acc) ->
                ...
                Tree = InitFun(RefKvs),
                ?assertEqual(?STRUCTURE_TEST_BASE_SIZE, xb5_trees_v2:size(Tree)),
